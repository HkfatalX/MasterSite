...
// ── Video Sources ──────────────────────────────────────────────
// Armazena as fontes do FILME no formulário (array de {type, label, url})
...
function detectSourceType(url) {
    ...
    if (u.includes('voe.sx') || u.includes('voesx')) return 'voe';
    if (u.includes('dood') || u.includes('doodstream')) return 'doodstream';
...
function sourceTypeLabel(type) {
    const labels = {
        mixdrop: 'MixDrop', streamtape: 'StreamTape', filemoon: 'FileMoon',
        voe: 'VOE', doodstream: 'DoodStream', mp4upload: 'Mp4Upload',
        okru: 'OK.ru', drive: 'Google Drive', yourupload: 'YourUpload', other: 'Outro'
    };
    return labels[type] || type;
}

// Normaliza fontes antigas (videoUrl string) para o novo formato [{type,label,url}]
function normalizeSources(item) {
    // Já tem videoSources no novo formato
    if (Array.isArray(item.videoSources) && item.videoSources.length > 0) {
        return item.videoSources;
    }
    // Compatibilidade: campo legado videoUrl
...
// Normaliza fontes de um episódio
...
window.onscroll = () => {
    ...
    if (h) { if (window.scrollY > 50) h.classList.add('scrolled'); else h.classList.remove('scrolled'); }
};
...
// ══════════════════════════════════════════════════════════════
// VIDEO SOURCES BUILDER (FILME)
// ══════════════════════════════════════════════════════════════
function getUsedSourceTypes() {
    return movieSourcesBuilder.map(s => s.type).filter(t => t && t !== 'other');
}
...
function renderMovieSources() {
    ...
    if (movieSourcesBuilder.length === 0) {
        container.innerHTML = '<div class="source-empty-state">Nenhuma fonte adicionada. Clique em <strong>+ Adicionar Fonte</strong>.</div>';
        return;
    }
    ...
    movieSourcesBuilder.forEach((src, idx) => {
        ...
        // Desabilita opções já usadas em outros índices (exceto o próprio tipo desta fonte e 'other')
        const usedTypes = movieSourcesBuilder
            .filter((_, i) => i !== idx)
            .map(s => s.type)
            .filter(t => t && t !== 'other');
        ...
        Array.from(typeSelect.options).forEach(opt => {
            if (opt.value && opt.value !== 'other' && opt.value !== src.type && usedTypes.includes(opt.value)) {
                opt.disabled = true;
                opt.text = opt.text + ' (já adicionado)';
            }
        });

        // Mostra badge do tipo selecionado
        if (src.type) {
            let preview = card.querySelector('.source-preview-tag');
            if (!preview) {
                preview = document.createElement('div');
                preview.className = 'source-preview-tag';
                card.appendChild(preview);
            }
            preview.textContent = `🔗 ${sourceTypeLabel(src.type)}`;
        }
        ...
        typeSelect.onchange = () => {
            ...
            // Verifica duplicata (exceto 'other' que pode repetir)
            if (newType && newType !== 'other') {
                const isDuplicate = movieSourcesBuilder.some((s, i) => i !== idx && s.type === newType);
                if (isDuplicate) {
                    showMsg(`Já existe uma fonte do tipo "${sourceTypeLabel(newType)}"! Escolha outro tipo.`, 'error');
                    typeSelect.value = src.type || '';
                    return;
                }
            }
            ...
            if (!movieSourcesBuilder[idx].label && newType) {
                movieSourcesBuilder[idx].label = sourceTypeLabel(newType);
                titleInput.value = movieSourcesBuilder[idx].label;
            }
            renderMovieSources();
        };
        ...
        // Evento: URL (detecta tipo automaticamente se tipo não selecionado)
        urlInput.oninput = () => {
            ...
            if (!movieSourcesBuilder[idx].type && urlInput.value) {
                ...
                if (detected && detected !== 'other') {
                    const isDuplicate = movieSourcesBuilder.some((s, i) => i !== idx && s.type === detected);
                    if (!isDuplicate) {
                        movieSourcesBuilder[idx].type = detected;
                        if (!movieSourcesBuilder[idx].label) {
                            movieSourcesBuilder[idx].label = sourceTypeLabel(detected);
                        }
                        renderMovieSources();
                    }
                }
        ...
        // Evento: remover
        removeBtn.onclick = () => {
            movieSourcesBuilder.splice(idx, 1);
            renderMovieSources();
        };
...
function addMovieSource() {
    movieSourcesBuilder.push({ type: '', label: '', url: '' });
    renderMovieSources();
}

const btnAddMovieSource = document.getElementById('btnAddMovieSource');
if (btnAddMovieSource) btnAddMovieSource.onclick = addMovieSource;

// ══════════════════════════════════════════════════════════════
// VIDEO SOURCES BUILDER (EPISÓDIOS DE SÉRIE)
// ══════════════════════════════════════════════════════════════
// episodeSourcesBuilder[si][ei] = [{type, label, url}]
let episodeSourcesBuilder = [];

function getEpUsedTypes(si, ei) {
    const sources = (episodeSourcesBuilder[si] && episodeSourcesBuilder[si][ei]) || [];
    return sources.map(s => s.type).filter(t => t && t !== 'other');
}
...
function renderEpisodeSources(si, ei, containerEl) {
    ...
    if (sources.length === 0) {
        containerEl.innerHTML = '<div class="source-empty-state" style="font-size:11px">Nenhuma fonte. Clique em + Fonte.</div>';
        return;
    }
    ...
    sources.forEach((src, sidx) => {
        ...
        // Desabilita tipos já usados neste episódio
        ...
        Array.from(typeSelect.options).forEach(opt => {
            if (opt.value && opt.value !== 'other' && opt.value !== src.type && usedTypes.includes(opt.value)) {
                opt.disabled = true;
                opt.text = opt.text.replace(' (já adicionado)', '') + ' (já adicionado)';
            }
        });
        ...
        typeSelect.onchange = () => {
            const newType = typeSelect.value;
            if (newType && newType !== 'other') {
                const isDuplicate = sources.some((s, i) => i !== sidx && s.type === newType);
                if (isDuplicate) {
                    showMsg(`Já existe uma fonte "${sourceTypeLabel(newType)}" neste episódio!`, 'error');
                    typeSelect.value = src.type || '';
                    return;
                }
            }
            episodeSourcesBuilder[si][ei][sidx].type = newType;
            if (!episodeSourcesBuilder[si][ei][sidx].label && newType) {
                episodeSourcesBuilder[si][ei][sidx].label = sourceTypeLabel(newType);
                titleInput.value = episodeSourcesBuilder[si][ei][sidx].label;
            }
        ...
        urlInput.oninput = () => {
            ...
            if (!episodeSourcesBuilder[si][ei][sidx].type && urlInput.value) {
                const detected = detectSourceType(urlInput.value);
                if (detected && detected !== 'other') {
                    const isDuplicate = sources.some((s, i) => i !== sidx && s.type === detected);
                    if (!isDuplicate) {
                        episodeSourcesBuilder[si][ei][sidx].type = detected;
                        if (!episodeSourcesBuilder[si][ei][sidx].label) {
                            episodeSourcesBuilder[si][ei][sidx].label = sourceTypeLabel(detected);
                        }
                        renderEpisodeSources(si, ei, containerEl);
                    }
                }
        ...
        removeBtn.onclick = () => {
            episodeSourcesBuilder[si][ei].splice(sidx, 1);
            renderEpisodeSources(si, ei, containerEl);
        };
...
if (btnConfirmCrop) btnConfirmCrop.onclick = () => {
...
if (btnCloseCropper) btnCloseCropper.onclick = () => {
    closeModal('cropperModal');
    if (previousModal) openModal(previousModal);
    if (cropperInstance) cropperInstance.destroy();
};
...
function renderSearchDropdown(query) {
    ...
    if (results.length === 0) {
        searchDropdown.innerHTML = `<div class="search-no-results"><span class="search-empty-icon">🔍</span><div>Nenhum resultado para "<strong>${query}</strong>"</div></div>`;
    } else {
...
if (mediaTypeEl) {
    mediaTypeEl.onchange = (e) => {
        ...
        if (t === 'movie') {
            ...
            if (lbl) lbl.innerText = "Duração (Ex: 2h 10m)";
        } else {
            ...
            const lbl = document.getElementById('mediaDurationLabel');
            if (lbl) lbl.innerText = "Duração Média (Ex: 45m/ep)";
...
function renderSeasonsBuilder() {
    ...
    seasonsBuilder.forEach((s, si) => {
        ...
        d.querySelector(`#seasonCoverBox_${si}`).onclick = () =>
            triggerCropModal(`seasonCoverFile_${si}`, `seasonCoverPrev_${si}`, 16 / 9);
        ...
        (s.episodes || []).forEach((ep, ei) => {
            ...
            ed.innerHTML = `
                ...
                <div class="input-group">
                    <label>Título</label>
                    <input type="text" id="epTitle_${si}_${ei}" value="${ep.title || ''}">
                </div>
                <div class="input-group">
                    <label>Duração</label>
                    <input type="text" id="epDuration_${si}_${ei}" value="${ep.duration || ''}">
                </div>
                <div class="input-group">
                    <label>Thumb</label>
                    <div class="file-upload-box" id="epThumbBox_${si}_${ei}">
                        <span class="file-upload-label">📸</span>
            ...
            ed.querySelector(`#epThumbBox_${si}_${ei}`).onclick = () =>
                triggerCropModal(`epThumbFile_${si}_${ei}`, `epThumbPrev_${si}_${ei}`, 16 / 9);
...
const btnAddSeason = document.getElementById('btnAddSeasonBtn');
...
function cleanExpiredContinue() {
    ...
    if (list.length !== b) saveContinueList(list);
    return list;
}
...
function setHero(item) {
    ...
    if (playBtn) playBtn.onclick = () => {
        if (item.type === 'movie') {
            ...
            initiatePlay(sources, item.title, 'Filme');
...
function openDetails(item) {
    ...
    if (detailMeta) detailMeta.innerText = `${item.type === 'movie' ? 'FILME' : 'SÉRIE'} • ${item.year || ''}${item.duration ? ' • ⏱️ ' + item.duration : ''}`;
    ...
    if (item.type === 'movie') {
        ...
        let rsi = 0, rei = -1;
        if (ci && ci.episodeInfo) {
            const match = ci.episodeInfo.match(/T(\d+)\s*E(\d+)/i);
            if (match) { rsi = parseInt(match[1]) - 1; rei = parseInt(match[2]) - 1; }
        }
        (item.seasons || []).forEach((s, idx) => {
            const tab = document.createElement('div');
            tab.className = `season-tab ${idx === rsi ? 'active' : ''}`;
            tab.innerText = `T${idx + 1}`; tab.tabIndex = 0;
            tab.onclick = () => {
        ...
        if (item.seasons?.length > 0) {
            const ti = item.seasons[rsi] ? rsi : 0;
            const ts = item.seasons[ti];
            if (ts.seasonCoverUrl?.trim() && ba) ba.style.backgroundImage = `url('${ts.seasonCoverUrl}')`;
            renderEpisodesList(ts.episodes || [], ti, ts, item, ti === rsi ? rei : -1);
        }
        openModal('detailsModal');
        if (ca) ca.scrollTop = 0;
        setTimeout(() => {
            const at = document.querySelector('.season-tab.active');
            if (at) { at.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' }); at.focus(); }
        }, 200);
    }
}
    ...
    eps.forEach((ep, idx) => {
        ...
        d.onkeydown = (e) => {
            if (e.key === 'Enter') d.click();
                ...
                e.preventDefault();
                const tabsArr = [...document.querySelectorAll('.season-tab')];
                const at = document.querySelector('.season-tab.active');
                if (at) { const ci2 = tabsArr.indexOf(at); let ni = e.key === 'ArrowRight' ? ci2 + 1 : ci2 - 1; if (ni >= 0 && ni < tabsArr.length) { tabsArr[ni].focus(); tabsArr[ni].click(); } }
            }
        };
        d.innerHTML = `
            <div class="episode-thumb">
                <img src="${th}" loading="lazy" onerror="this.src='${fi}'">
                <div class="ep-play-overlay"><span>▶</span></div>
            </div>
            <div style="flex:1;min-width:0">
                <div style="font-size:13px;font-weight:800">${lbl} - ${ep.title || 'Sem Título'}${dur}</div>
                <span style="font-size:11px;color:var(--primary-color);font-weight:700">▶ ${isR ? 'Continuar' : 'Assistir'}</span>
                ${rb}
            </div>`;
...
if (btnCloseDetails) btnCloseDetails.onclick = () => { window.location.hash = ''; closeModal('detailsModal'); };

// ══════════════════════════════════════════════════════════════
// INITIATE PLAY — decide se mostra seletor ou vai direto
// ══════════════════════════════════════════════════════════════
function initiatePlay(sources, mainTitle, subTitle) {
    if (!sources || sources.length === 0) {
        showMsg('Nenhum link de vídeo disponível!', 'error');
        return;
    }

    // Apenas 1 fonte → play direto, sem seletor
    if (sources.length === 1) {
        openModal('playerModal');
        playVideo(sources[0].url, mainTitle, subTitle);
        return;
    }

    // Mais de 1 fonte → abre player e mostra seletor
    openModal('playerModal');
    showSourceSelector(sources, mainTitle, subTitle);
}

// ══════════════════════════════════════════════════════════════
// SOURCE SELECTOR (dentro do player)
// ══════════════════════════════════════════════════════════════
function showSourceSelector(sources, mainTitle, subTitle) {
    ...
    // Esconde loading enquanto o seletor está visível
    if (loading) loading.classList.add('hidden');

    // Atualiza título do player
    const titleDisp = document.getElementById('playerTitleDisplay');
    if (titleDisp) titleDisp.innerText = mainTitle || 'Escolha uma fonte';
    const subDisp = document.getElementById('playerSubDisplay');
    if (subDisp) subDisp.innerText = subTitle || 'MasterFlix';

    if (subEl) subEl.textContent = `${sources.length} fontes disponíveis — escolha uma para iniciar.`;
    ...
    sources.forEach((src, idx) => {
        ...
        const title = src.label || sourceTypeLabel(src.type) || `Fonte ${idx + 1}`;
        const typeLbl = src.type ? sourceTypeLabel(src.type) : 'Outro';
        const shortUrl = src.url ? (src.url.length > 50 ? src.url.substring(0, 50) + '…' : src.url) : '';

        btn.innerHTML = `
            <div class="source-btn-top">
                <span class="source-btn-title">${title}</span>
                <span class="source-btn-type">${typeLbl}</span>
            </div>
            <div class="source-btn-url">${shortUrl}</div>`;

        btn.onclick = () => {
            hideSourceSelector();
            playVideo(src.url, mainTitle, subTitle);
        };
        btn.onkeydown = (e) => {
    ...
    // Foca no primeiro botão (TV/acessibilidade)
...
// Botão cancelar seleção → fecha o player inteiro
const btnCancelSourceSelection = document.getElementById('btnCancelSourceSelection');
if (btnCancelSourceSelection) btnCancelSourceSelection.onclick = () => closePlayer();
    ...
    overlayCleanerInterval = setInterval(() => {
        ...
        try {
            ...
            doc.querySelectorAll('div, aside, section, span').forEach(el => {
                ...
                if (isOverlay && isFullCover && !el.querySelector('video')) {
...
if (btnToggleFullscreen) btnToggleFullscreen.onclick = (e) => {
    e.stopPropagation();
    if (!isInFullscreen()) {
...
if (btnToggleStretch) btnToggleStretch.onclick = (e) => {
    e.stopPropagation();
    if (playerContainer) {
        playerContainer.classList.toggle('stretch-mode');
        showMsg(playerContainer.classList.contains('stretch-mode') ? '📐 Esticado' : '🖼️ Normal', 'success');
    }
    showPlayerControls();
};

const btnReloadPlayer = document.getElementById('btnReloadPlayer');
if (btnReloadPlayer) btnReloadPlayer.onclick = (e) => {
    e.stopPropagation();
...
function playVideo(rawUrl, mainTitle, subTitle) {
    ...
    const titleDisp = document.getElementById('playerTitleDisplay');
    if (titleDisp) titleDisp.innerText = mainTitle || 'Assistindo';
    const subDisp = document.getElementById('playerSubDisplay');
    if (subDisp) subDisp.innerText = subTitle || 'MasterFlix';
    ...
    setTimeout(() => {
        ...
        if (tip && isPlayerOpen()) { tip.classList.add('show'); setTimeout(() => tip.classList.remove('show'), 5000); }
    }, 3000);
    setTimeout(() => playerLoading?.classList.add('hidden'), 6000);

    const audioScanInterval = setInterval(() => {
        if (!isPlayerOpen()) { clearInterval(audioScanInterval); return; }
        scanAndBoostAudio();
    }, 5000);
}
...
function closePlayer() {
    ...
    const booster = document.getElementById('audioBoostSlider');
    if (booster) booster.value = 100;
    const boostVal = document.getElementById('audioBoostValue');
    if (boostVal) boostVal.textContent = '100%';
}

const btnClosePlayer = document.getElementById('btnClosePlayer');
if (btnClosePlayer) btnClosePlayer.onclick = (e) => { e.stopPropagation(); closePlayer(); };
...
document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'ArrowUp') {
        e.preventDefault();
        const sl = document.getElementById('audioBoostSlider');
        if (sl) { sl.value = Math.min(500, parseInt(sl.value) + 25); setAudioBoost(parseInt(sl.value)); }
    }
    if (e.key === 'ArrowDown') {
        e.preventDefault();
        const sl = document.getElementById('audioBoostSlider');
        if (sl) { sl.value = Math.max(100, parseInt(sl.value) - 25); setAudioBoost(parseInt(sl.value)); }
    }
    showPlayerControls();
});

// ══════════════════════════════════════════════════════════════
// FORM SUBMIT — salva no Firebase
// ══════════════════════════════════════════════════════════════
...
if (mediaForm) {
    mediaForm.onsubmit = async (e) => {
        ...
        if (type === 'movie') {
            ...
            // Checa duplicatas de tipo
            const usedTypes = validSources.map(s => s.type).filter(t => t && t !== 'other');
            const duplicateType = usedTypes.find((t, i) => usedTypes.indexOf(t) !== i);
            if (duplicateType) { showMsg(`Tipo duplicado: ${sourceTypeLabel(duplicateType)}. Remova a duplicata.`, 'error'); return; }

            payload.videoSources = validSources.map(s => ({
                type: s.type || 'other',
                label: s.label || sourceTypeLabel(s.type) || 'Fonte',
                url: formatVideoUrl(s.url.trim())
            }));
            // Mantém campo legado para compatibilidade com dados antigos
            payload.videoUrl = payload.videoSources[0].url;

        } else {
            // Série: monta seasons com videoSources em cada episódio
            const us = [];
            for (let si = 0; si < seasonsBuilder.length; si++) {
                ...
                for (let ei = 0; ei < (s.episodes || []).length; ei++) {
                    ...
                    if (epDupType) {
                        showMsg(`T${si+1} E${ei+1}: tipo "${sourceTypeLabel(epDupType)}" duplicado!`, 'error');
                        return;
                    }
                    ...
                    ue.push({
                        ...
                        thumbUrl: et,
                        videoSources: validEpSources.map(src => ({
                            type: src.type || 'other',
                            label: src.label || sourceTypeLabel(src.type) || 'Fonte',
                            url: formatVideoUrl(src.url.trim())
                        })),
                        // Compatibilidade legada
                        videoUrl: validEpSources.length > 0 ? formatVideoUrl(validEpSources[0].url.trim()) : ''
                    });
                }
...
window.editMedia = (id) => {
    ...
    setVal('mediaTitle', i.title);
    setVal('mediaYear', i.year);
    setVal('mediaDuration', i.duration);
    setVal('mediaDesc', i.description);
    ...
    if (i.type === 'movie') {
        ...
        movieSourcesBuilder = normalizeSources(i).map(s => ({ ...s }));
        renderMovieSources();
    }
    ...
    if (i.type === 'serie') {
        ...
        episodeSourcesBuilder = [];
        // Carrega fontes de cada episódio
        seasonsBuilder.forEach((s, si) => {
            episodeSourcesBuilder[si] = [];
            (s.episodes || []).forEach((ep, ei) => {
                episodeSourcesBuilder[si][ei] = normalizeEpisodeSources(ep).map(src => ({ ...src }));
            });
        });
        renderSeasonsBuilder();
    }
...
if (adminSearch) adminSearch.oninput = renderAdminCatalogList;
const btnCloseAdmin = document.getElementById('btnCloseAdmin');
if (btnCloseAdmin) btnCloseAdmin.onclick = () => closeModal('adminModal');
const btnAddNewAdmin = document.getElementById('btnAddNewFromAdmin');
if (btnAddNewAdmin) btnAddNewAdmin.onclick = () => { closeModal('adminModal'); openCreator(); };
...
if (btnCloseSuggestion) btnCloseSuggestion.onclick = () => closeModal('suggestionModal');
...
// SUGGESTIONS ADMIN
// ══════════════════════════════════════════════════════════════
const btnCloseSuggAdmin = document.getElementById('btnCloseSuggestionsAdmin');
if (btnCloseSuggAdmin) btnCloseSuggAdmin.onclick = () => { exitSelectMode(); closeModal('suggestionsAdminModal'); };
...
async function loadSuggestionsAdmin() {
    try {
        ...
        const txt = document.getElementById('suggestionsCountText');
        if (txt) txt.textContent = `${allSuggestions.length} item(s)`;
...
function renderSuggestionsList() {
    ...
    allSuggestions.forEach(s => {
        ...
        if (suggestionsSelectMode) {
            ...
            if (chk) chk.onchange = () => { if (chk.checked) selectedSuggestionIds.add(s.id); else selectedSuggestionIds.delete(s.id); };
        }
        c.appendChild(d);
    });
}
...
document.getElementById('btnDeleteSelectedSuggestions')?.addEventListener('click', async () => {
    ...
    if (!confirm(`Apagar ${selectedSuggestionIds.size}?`)) return;
    try {
        for (let id of selectedSuggestionIds) await remove(ref(rtdb, "suggestions/" + id));
        showMsg('OK! ✅', 'success'); exitSelectMode(); loadSuggestionsAdmin();
    } catch (e) { showMsg('Erro', 'error'); }
});
window.deleteSingleSuggestion = async (id) => {
    if (!confirm('Apagar?')) return;
    try { await remove(ref(rtdb, "suggestions/" + id)); loadSuggestionsAdmin(); } catch (e) { }
};
...
if (btnCloseStorage) btnCloseStorage.onclick = () => closeModal('storageModal');
    ...
    try {
        ...
        c.innerHTML = `
            ...
            <div style="display:flex;gap:10px;margin-bottom:22px">
                <div class="storage-icon-card"><span class="sto-icon">🎬</span><div class="sto-count">${movieCount}</div><div class="sto-label">Filmes</div></div>
                <div class="storage-icon-card"><span class="sto-icon">📺</span><div class="sto-count">${serieCount}</div><div class="sto-label">Séries</div></div>
                <div class="storage-icon-card"><span class="sto-icon">🎞️</span><div class="sto-count">${totalEpisodes}</div><div class="sto-label">Episódios</div></div>
            </div>
    ...
    try {
        ...
        if (s.exists()) {
            ...
            if (data.themeColor && !isTVDevice()) {
                ...
                const p = document.getElementById('themeColorPicker');
                if (p) p.value = data.themeColor;
            }
...
function resetProfileUI() {
    ...
    const pBanner = document.getElementById('profileBannerImg');
    if (pBanner) pBanner.src = 'https://via.placeholder.com/600x200?text=Banner';
    ...
    if (sAvatar) sAvatar.innerHTML = '<span>U</span>';
...
function updateUserAvatarUI(data = {}) {
    ...
    const setTxt = (id, txt) => { const el = document.getElementById(id); if (el) el.innerText = txt; };
    if (un) {
        setTxt('profileNameDisplay', un);
        const pName = document.getElementById('profileNameInput'); if (pName) pName.value = un;
        setTxt('sidebarUserName', un);
    } else if (user) {
        const name = user.email.split('@')[0];
        setTxt('sidebarUserName', name); setTxt('profileNameDisplay', name);
    }
    ...
    if (bn) {
        const bImg = document.getElementById('profileBannerImg'); if (bImg) bImg.src = bn;
        const bPrev = document.getElementById('profileBannerPreview'); if (bPrev) { bPrev.src = bn; bPrev.classList.remove('hidden'); }
    }
...
if (toggleAuth) {
    toggleAuth.onclick = () => {
        ...
        if (sub) sub.innerText = isSignUpMode ? 'Crie sua conta gratuita' : 'Entre na sua conta para continuar';
        const btn = document.getElementById('btnAuthSubmit');
        if (btn) btn.innerText = isSignUpMode ? 'Criar Conta' : 'Entrar na Conta';
...
onAuthStateChanged(auth, (user) => {
    if (user) {
        ...
        isSignUpMode = false;
        const sub = document.getElementById('authSubtitle');
        if (sub) sub.innerText = 'Entre na sua conta para continuar';
        const btn = document.getElementById('btnAuthSubmit');
        if (btn) btn.innerText = 'Entrar na Conta';
