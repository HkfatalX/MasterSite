...
function getEpisodeThumb(ep,sd,si){if(ep.thumbUrl&&ep.thumbUrl.trim()!==''&&!ep.thumbUrl.includes('window.location'))return ep.thumbUrl;if(sd&&sd.seasonCoverUrl&&sd.seasonCoverUrl.trim()!==''&&!sd.seasonCoverUrl.includes('window.location'))return sd.seasonCoverUrl;return getBackdropUrl(si)||getPosterUrl(si);}
...
function translateAuthError(c){const e={'auth/email-already-in-use':'Este e-mail já está cadastrado!','auth/invalid-email':'E-mail inválido.','auth/weak-password':'Senha fraca. Mínimo 6 caracteres.','auth/user-not-found':'Conta não encontrada.','auth/wrong-password':'Senha incorreta.','auth/invalid-credential':'E-mail ou senha incorretos.','auth/too-many-requests':'Muitas tentativas. Aguarde.','auth/network-request-failed':'Sem internet.','auth/user-disabled':'Conta desativada.'};return e[c]||`Erro: ${c}`;}
...
function loadUserTheme(){if(isTVDevice())return;const uc=getUserCache('theme_color');const gc=localStorage.getItem('masterflix_theme_color');const color=uc||gc||'#e50914';document.documentElement.style.setProperty('--primary-color',color);document.getElementById('themeColorPicker').value=color;}
...
// ========== SIDEBAR ==========
const menuToggleBtn=document.getElementById('menuToggleBtn');const sidebarMenu=document.getElementById('sidebarMenu');const sidebarOverlay=document.getElementById('sidebarOverlay');
...
// ========== GENRE SELECTOR ==========
function renderGenreSelector(){const c=document.getElementById('genreSelectorContainer');c.innerHTML='';AVAILABLE_GENRES.forEach(g=>{const t=document.createElement('div');t.className='genre-tag';if(selectedGenres.includes(g))t.classList.add('selected');t.textContent=g;t.onclick=()=>{const i=selectedGenres.indexOf(g);if(i>=0)selectedGenres.splice(i,1);else selectedGenres.push(g);renderGenreSelector();};c.appendChild(t);});const ct=document.getElementById('genreCounter');if(selectedGenres.length===0){ct.textContent='⚠️ Selecione ao menos 1';ct.style.color='#ff9800';}else{ct.textContent=`✓ ${selectedGenres.length}: ${selectedGenres.join(', ')}`;ct.style.color='var(--primary-color)';}}

// ========== CROPPER ==========
window.triggerCropModal=function(inputId,previewId,ar){const fi=document.getElementById(inputId);currentTargetInput=fi;currentTargetPreview=document.getElementById(previewId);fi.onchange=(e)=>{const file=e.target.files[0];if(!file)return;const reader=new FileReader();reader.onload=(ev)=>{const tmp=new Image();tmp.onload=()=>{if(tmp.naturalWidth<=800&&tmp.naturalHeight<=800){if(currentTargetPreview){currentTargetPreview.src=ev.target.result;currentTargetPreview.classList.remove('hidden');}fi.value="";return;}document.getElementById('cropperImage').src=ev.target.result;if(!document.getElementById('profileModal').classList.contains('hidden')){previousModal='profileModal';document.getElementById('profileModal').classList.add('hidden');}else if(!document.getElementById('creatorModal').classList.contains('hidden')){previousModal='creatorModal';document.getElementById('creatorModal').classList.add('hidden');}document.getElementById('cropperModal').classList.remove('hidden');if(cropperInstance)cropperInstance.destroy();cropperInstance=new Cropper(document.getElementById('cropperImage'),{aspectRatio:ar||NaN,viewMode:1,autoCropArea:1,responsive:true,restore:false,center:true,highlight:false,crop(){const cv=cropperInstance.getCroppedCanvas({width:800,imageSmoothingQuality:'high'});if(cv){const u=cv.toDataURL('image/jpeg',0.90);document.getElementById('prevMobile').src=u;document.getElementById('prevPC').src=u;document.getElementById('prevTV').src=u;}}});};tmp.src=ev.target.result;};reader.readAsDataURL(file);};fi.click();};
document.getElementById('btnConfirmCrop').onclick=()=>{if(!cropperInstance)return;const cv=cropperInstance.getCroppedCanvas({width:1200,imageSmoothingQuality:'high'});if(cv&&currentTargetPreview){currentTargetPreview.src=cv.toDataURL('image/jpeg',0.92);currentTargetPreview.classList.remove('hidden');}document.getElementById('cropperModal').classList.add('hidden');if(previousModal)document.getElementById(previousModal).classList.remove('hidden');if(cropperInstance)cropperInstance.destroy();};
...
// ========== SEARCH ==========
const searchWrapper=document.getElementById('searchWrapper');const searchBox=document.getElementById('searchBox');const searchInput=document.getElementById('searchInput');const searchDropdown=document.getElementById('searchResultsDropdown');
...
function renderSearchDropdown(query){const results=smartSearch(query);searchDropdown.innerHTML='';if(results.length===0){searchDropdown.innerHTML=`<div class="search-no-results">🔍 Nada encontrado</div>`;}else{results.forEach(item=>{const d=document.createElement('div');d.className='search-result-item';d.tabIndex=0;const p=getPosterUrl(item)||'https://via.placeholder.com/50x70?text=?';const tl=item.type==='movie'?'Filme':'Série';let gt='';if(Array.isArray(item.genres)&&item.genres.length>0)gt=item.genres.slice(0,2).join(', ');else if(item.category)gt=item.category;d.innerHTML=`<img class="search-result-thumb" src="${p}" onerror="this.src='https://via.placeholder.com/50x70?text=?'"><div class="search-result-info"><div class="search-result-title">${highlightMatch(item.title,query)}</div><div class="search-result-meta"><span class="tag">${tl}</span>${gt?' • '+gt:''}${item.year?' • '+item.year:''}</div></div>`;d.onclick=()=>{searchDropdown.classList.remove('visible');searchInput.value='';searchBox.classList.remove('has-text');closeSearch();window.location.hash=`#/midia/${item.id}`;openDetails(item);};searchDropdown.appendChild(d);});}searchDropdown.classList.add('visible');}
...
// ========== MEDIA TYPE ==========
document.getElementById('mediaType').onchange=(e)=>{const t=e.target.value;if(t==='movie'){document.getElementById('movieFileArea').classList.remove('hidden');document.getElementById('seriesBuilderArea').classList.add('hidden');document.getElementById('mediaDurationLabel').innerText="Duração (Ex: 2h 10m)";document.getElementById('mediaDuration').placeholder="Ex: 2h 10m";}else{document.getElementById('movieFileArea').classList.add('hidden');document.getElementById('seriesBuilderArea').classList.remove('hidden');document.getElementById('mediaDurationLabel').innerText="Duração Média (Ex: 45m/ep)";document.getElementById('mediaDuration').placeholder="Ex: 45m/ep";if(seasonsBuilder.length===0)addSeason();}};

// ========== SEASONS BUILDER ==========
function renderSeasonsBuilder(){const c=document.getElementById('seasonsList');c.innerHTML="";seasonsBuilder.forEach((s,si)=>{const d=document.createElement('div');d.style.cssText="background:#181818;border:1px solid #333;border-radius:8px;padding:12px;margin-top:12px;";d.innerHTML=`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;"><strong style="color:var(--primary-color);font-size:13px;">Temporada ${si+1}</strong><button type="button" class="btn-secondary" style="font-size:10px;padding:4px 8px;" onclick="addEpisode(${si})">+ Episódio</button></div><div class="input-group" style="margin-bottom:8px;"><label style="font-size:10px;">Foto Temporada (16:9)</label><div class="file-upload-box" onclick="triggerCropModal('seasonCoverFile_${si}','seasonCoverPrev_${si}',16/9)"><span class="file-upload-label">📁 Foto</span><input type="file" id="seasonCoverFile_${si}" accept="image/*" class="hidden"><img id="seasonCoverPrev_${si}" src="${s.seasonCoverUrl||''}" class="file-preview-img ${s.seasonCoverUrl?'':'hidden'}"></div></div><div id="episodesListBuilder_${si}"></div>`;c.appendChild(d);const el=d.querySelector(`#episodesListBuilder_${si}`);(s.episodes||[]).forEach((ep,ei)=>{const ed=document.createElement('div');ed.style.cssText="background:#111;border:1px solid #282828;border-radius:6px;padding:10px;margin-top:10px;";ed.innerHTML=`<strong style="font-size:10px;color:#aaa;">EP ${ei+1}</strong><div class="input-group" style="margin-top:4px;margin-bottom:6px;"><label style="font-size:9px;">Título</label><input type="text" id="epTitle_${si}_${ei}" value="${ep.title||''}" placeholder="Nome"></div><div class="input-group" style="margin-bottom:6px;"><label style="font-size:9px;">Duração</label><input type="text" id="epDuration_${si}_${ei}" value="${ep.duration||''}" placeholder="45m"></div><div class="input-group" style="margin-bottom:6px;"><label style="font-size:9px;">Thumb (16:9)</label><div class="file-upload-box" onclick="triggerCropModal('epThumbFile_${si}_${ei}','epThumbPrev_${si}_${ei}',16/9)"><span class="file-upload-label">📸</span><input type="file" id="epThumbFile_${si}_${ei}" accept="image/*" class="hidden"><img id="epThumbPrev_${si}_${ei}" src="${ep.thumbUrl||''}" class="file-preview-img ${ep.thumbUrl?'':'hidden'}"></div></div><div class="input-group" style="margin-bottom:2px;"><label style="font-size:9px;">Link Vídeo</label><input type="text" id="epVideoUrl_${si}_${ei}" value="${ep.videoUrl||''}" placeholder="Mixdrop"></div>`;el.appendChild(ed);});});}
...
// ========== CONTINUE WATCHING (SEM TIMER) ==========
function getContinueStorageKey(){if(!currentUserUid)return null;return`mfx_${currentUserUid}_continue`;}
function getContinueList(){const key=getContinueStorageKey();if(!key)return[];try{return JSON.parse(localStorage.getItem(key)||'[]');}catch(e){return[];}}
function saveContinueList(list){const key=getContinueStorageKey();if(!key)return;localStorage.setItem(key,JSON.stringify(list));}

function cleanExpiredContinue(){
    let list=getContinueList();
    const now=Date.now();
    const before=list.length;
    list=list.filter(item=>(now-(item.lastWatched||item.timestamp||0))<CONTINUE_EXPIRE_MS);
...
function saveContinueWatching(mi,extra='',episodeInfo=null){
    ...
    try{
        ...
        const now=Date.now();
        cl=cl.filter(item=>(now-(item.lastWatched||item.timestamp||0))<CONTINUE_EXPIRE_MS);
...
function removeContinueItem(mediaId){
    if(!currentUserUid)return;
    let cl=getContinueList();
    cl=cl.filter(i=>i.id!==mediaId);
    saveContinueList(cl);
    renderContinueWatching();
}

// Busca info do continue watching para um item específico
function getContinueInfo(mediaId){
    const list=getContinueList();
    return list.find(i=>i.id===mediaId)||null;
}
...
function renderContinueWatching(){
    ...
    if(selectedCategory==="Filmes")list=list.filter(i=>i.type==='movie');
        ...
        list=list.filter(i=>{const o=mediaCatalog.find(m=>m.id===i.id);if(!o)return false;return getItemGenres(o).includes(selectedCategory);});
    }
    ...
    list.forEach(item=>{
        ...
        card.innerHTML=`
            <img src="${item.coverUrl||'https://via.placeholder.com/300x180?text=?'}" alt="${item.title}">
            <button class="continue-remove-btn" title="Remover" data-id="${item.id}">✕</button>
...
function renderApp(){
    ...
    mediaCatalog.forEach(item=>{if(!itemMatchesCategory(item,selectedCategory))return;const card=document.createElement('div');card.className='media-card';card.tabIndex=0;const ps=getPosterUrl(item),gs=getItemGenres(item),tg=gs.length>0?gs[0]:(item.type==='movie'?'Filme':'Série');if(ps){card.innerHTML=`<img class="media-card-poster" src="${ps}" alt="${item.title}" loading="lazy" onerror="this.style.display='none';this.parentElement.querySelector('.media-card-fallback-hidden').style.display='flex';"><div class="media-card-poster-fallback media-card-fallback-hidden" style="display:none;position:absolute;inset:0;">🎬</div><div class="media-card-overlay"><span class="media-card-tag">${tg}</span><div class="media-card-title">${item.title}</div></div>`;}else{card.innerHTML=`<div class="media-card-poster-fallback">🎬</div><div class="media-card-overlay"><span class="media-card-tag">${tg}</span><div class="media-card-title">${item.title}</div></div>`;}card.onclick=()=>{window.location.hash=`#/midia/${item.id}`;openDetails(item);};if(item.type==='movie')mc.appendChild(card);else sc.appendChild(card);});
    renderContinueWatching();
    if(mediaCatalog.length>0&&!activeItem)setHero(mediaCatalog[0]);
}

function setHero(item){activeItem=item;document.getElementById('heroTitle').innerText=item.title;document.getElementById('heroDesc').innerText=item.description;const gs=getItemGenres(item),tl=item.type==='movie'?'🎬 FILME':'📺 SÉRIE';document.getElementById('heroMeta').innerHTML=`<strong>${tl}</strong>${item.year?` <span class="dot">•</span> ${item.year}`:''}${item.duration?` <span class="dot">•</span> ⏱️ ${item.duration}`:''}${gs.length>0?` <span class="dot">•</span> ${gs.slice(0,3).join(', ')}`:''}`;const bd=getBackdropUrl(item);if(bd)document.getElementById('heroBackdrop').style.backgroundImage=`url('${bd}')`;document.getElementById('heroPlayBtn').onclick=()=>{if(item.type==='movie'&&item.videoUrl){saveContinueWatching(item,item.duration||'');playVideo(item.videoUrl,item.title,'Filme');}else openDetails(item);};document.getElementById('heroInfoBtn').onclick=()=>openDetails(item);}
...
function openDetails(item){
    ...
    // Busca info de "continuar assistindo"
    ...
    if(item.type==='movie'){
        ...
        const btnText=continueInfo?'▶ Continuar Assistindo':'▶ Assistir Filme';
        document.getElementById('btnPlayMovieFile').innerText=btnText;
        document.getElementById('btnPlayMovieFile').onclick=()=>{saveContinueWatching(item,item.duration||'');playVideo(item.videoUrl,item.title,'Filme');};
    }else{
        ...
        // Descobre em qual temporada/episódio parou
        let resumeSeasonIdx=0;
        let resumeEpIdx=-1;
        if(continueInfo&&continueInfo.episodeInfo){
            // Tenta parsear "T1 E3 - Nome"
        ...
        (item.seasons||[]).forEach((s,idx)=>{
            const tab=document.createElement('div');tab.className=`season-tab ${idx===resumeSeasonIdx?'active':''}`;
            tab.innerText=`Temporada ${idx+1}`;
            tab.onclick=()=>{document.querySelectorAll('.season-tab').forEach(t=>t.classList.remove('active'));tab.classList.add('active');if(s.seasonCoverUrl?.trim())ba.style.backgroundImage=`url('${s.seasonCoverUrl}')`;else ba.style.backgroundImage=bd?`url('${bd}')`:``; renderEpisodesList(s.episodes||[],idx,s,item,idx===resumeSeasonIdx?resumeEpIdx:-1);};
        ...
        if(item.seasons?.length>0){
            ...
            if(targetSeason.seasonCoverUrl?.trim())ba.style.backgroundImage=`url('${targetSeason.seasonCoverUrl}')`;
...
function renderEpisodesList(eps,si,sd,ser,resumeEpIdx){
    ...
    eps.forEach((ep,idx)=>{
        ...
        const th=getEpisodeThumb(ep,sd,ser);
        ...
        d.innerHTML=`
            <div class="episode-thumb"><img src="${th||'https://via.placeholder.com/150x90?text=?'}" onerror="this.src='https://via.placeholder.com/150x90?text=?'"></div>
            <div style="flex:1;">
                <div style="font-size:12px;font-weight:800;">${epLabel} - ${ep.title||'Sem Título'}${dur}</div>
                <span style="font-size:10px;color:var(--primary-color);font-weight:700;">▶ ${isResumePoint?'Continuar':'Assistir'}</span>
                ${resumeBadge}
            </div>
        `;

        // Destaca visualmente onde parou
        if(isResumePoint){
            d.style.border='1px solid rgba(255,202,40,0.4)';
            d.style.background='rgba(255,202,40,0.05)';
        }

        c.appendChild(d);
    });

    // Se tem ponto de resumo, scrolla até ele
    if(resumeEpIdx>=0){
        setTimeout(()=>{
            const cards=c.querySelectorAll('.episode-card');
            if(cards[resumeEpIdx])cards[resumeEpIdx].scrollIntoView({behavior:'smooth',block:'nearest'});
        },100);
    }
}
...
// ========== FORM SUBMIT ==========
document.getElementById('mediaForm').onsubmit=async(e)=>{e.preventDefault();if(selectedGenres.length===0){showMsg('Selecione ao menos 1 gênero!','error');return;}const editId=document.getElementById('editMediaId').value,type=document.getElementById('mediaType').value;let cv=document.getElementById('mediaCoverPreview').src,bd=document.getElementById('mediaBackdropPreview').src;if((!cv||cv.includes('window.location'))&&editId){const ex=mediaCatalog.find(m=>m.id===editId);if(ex)cv=ex.coverUrl;}if((!bd||bd.includes('window.location'))&&editId){const ex=mediaCatalog.find(m=>m.id===editId);if(ex)bd=ex.backdropUrl;}let payload={type,genres:[...selectedGenres],category:selectedGenres[0],title:document.getElementById('mediaTitle').value.trim(),year:document.getElementById('mediaYear').value.trim(),duration:document.getElementById('mediaDuration').value.trim(),description:document.getElementById('mediaDesc').value.trim(),coverUrl:(cv&&!cv.includes('window.location'))?cv:'',backdropUrl:(bd&&!bd.includes('window.location'))?bd:''};if(type==='movie')payload.videoUrl=document.getElementById('movieVideoUrl').value.trim();else{const us=[];for(let si=0;si<seasonsBuilder.length;si++){const s=seasonsBuilder[si];let sc=document.getElementById(`seasonCoverPrev_${si}`)?.src||(s.seasonCoverUrl||'');if(sc.includes('window.location'))sc='';const ue=[];for(let ei=0;ei<(s.episodes||[]).length;ei++){const ep=s.episodes[ei];let et=document.getElementById(`epThumbPrev_${si}_${ei}`)?.src||(ep.thumbUrl||'');if(et.includes('window.location'))et='';ue.push({title:document.getElementById(`epTitle_${si}_${ei}`)?.value?.trim()||`Episódio ${ei+1}`,duration:document.getElementById(`epDuration_${si}_${ei}`)?.value?.trim()||'',videoUrl:document.getElementById(`epVideoUrl_${si}_${ei}`)?.value?.trim()||'',thumbUrl:et});}us.push({seasonNumber:s.seasonNumber||si+1,seasonCoverUrl:sc,episodes:ue});}payload.seasons=us;}try{if(editId){await set(ref(rtdb,"catalog/"+editId),payload);showMsg('Atualizado!','success');}else{await set(push(ref(rtdb,"catalog")),payload);showMsg('Publicado!','success');}document.getElementById('creatorModal').classList.add('hidden');document.getElementById('mediaForm').reset();document.getElementById('mediaCoverPreview').classList.add('hidden');document.getElementById('mediaBackdropPreview').classList.add('hidden');seasonsBuilder=[];selectedGenres=[];await loadCatalog();}catch(err){showMsg('Erro: '+err.message,'error');}};

// ========== ADMIN ==========
window.editMedia=(id)=>{const i=mediaCatalog.find(m=>m.id===id);if(!i)return;document.getElementById('editMediaId').value=i.id;const ts=document.getElementById('mediaType');ts.value=i.type;ts.disabled=true;ts.style.cursor='not-allowed';ts.style.opacity='0.6';ts.dispatchEvent(new Event('change'));selectedGenres=Array.isArray(i.genres)&&i.genres.length>0?[...i.genres]:(i.category?[i.category]:[]);renderGenreSelector();document.getElementById('mediaTitle').value=i.title;document.getElementById('mediaYear').value=i.year;document.getElementById('mediaDuration').value=i.duration||'';document.getElementById('mediaDesc').value=i.description;document.getElementById('movieVideoUrl').value=i.videoUrl||'';if(i.coverUrl){document.getElementById('mediaCoverPreview').src=i.coverUrl;document.getElementById('mediaCoverPreview').classList.remove('hidden');}if(i.backdropUrl){document.getElementById('mediaBackdropPreview').src=i.backdropUrl;document.getElementById('mediaBackdropPreview').classList.remove('hidden');}document.getElementById('creatorTitle').innerText="Editar";if(i.type==='serie'){seasonsBuilder=JSON.parse(JSON.stringify(i.seasons||[]));renderSeasonsBuilder();}document.getElementById('adminModal').classList.add('hidden');document.getElementById('creatorModal').classList.remove('hidden');};
...
function renderAdminCatalogList(){const c=document.getElementById('adminCatalogList'),sv=normalizeText(document.getElementById('adminSearchInput').value);c.innerHTML="";mediaCatalog.forEach(i=>{if(sv&&!normalizeText(i.title).includes(sv))return;const d=document.createElement('div');d.className='admin-item';d.innerHTML=`<div><strong style="font-size:13px;">${i.title}</strong><div style="font-size:10px;color:#aaa;">${i.type==='movie'?'Filme':'Série'} • ${getItemGenres(i).join(', ')}</div></div><div style="display:flex;gap:6px;"><button class="btn-secondary" style="font-size:11px;padding:5px 10px;" onclick="editMedia('${i.id}')">✏️</button><button class="btn-secondary" style="font-size:11px;padding:5px 10px;color:#ff5252;" onclick="deleteMedia('${i.id}')">🗑️</button></div>`;c.appendChild(d);});}
...
function openCreator(){document.getElementById('editMediaId').value="";document.getElementById('mediaForm').reset();const ts=document.getElementById('mediaType');ts.disabled=false;ts.style.cursor='pointer';ts.style.opacity='1';ts.dispatchEvent(new Event('change'));document.getElementById('movieVideoUrl').value="";document.getElementById('mediaCoverPreview').classList.add('hidden');document.getElementById('mediaBackdropPreview').classList.add('hidden');document.getElementById('creatorTitle').innerText="Publicar";seasonsBuilder=[];selectedGenres=[];renderGenreSelector();document.getElementById('seasonsList').innerHTML="";addSeason();document.getElementById('creatorModal').classList.remove('hidden');}

// ========== SUGGESTIONS ==========
document.getElementById('btnCloseSuggestion').onclick=()=>document.getElementById('suggestionModal').classList.add('hidden');
document.getElementById('btnSendSuggestion').onclick=async()=>{const user=auth.currentUser;if(!user){showMsg('Faça login!','error');return;}const text=document.getElementById('suggestionText').value.trim();const type=document.getElementById('suggestionType').value;if(!text||text.length<5){showMsg('Escreva algo válido!','error');return;}try{const userName=getUserCache('name')||user.email.split('@')[0];await set(push(ref(rtdb,"suggestions")),{userId:user.uid,userEmail:user.email,userName,text,type,timestamp:Date.now()});document.getElementById('suggestionText').value='';showMsg(type==='bug'?'Bug reportado! 🐛':'Sugestão enviada! 💡','success');document.getElementById('suggestionModal').classList.add('hidden');}catch(e){showMsg('Erro: '+e.message,'error');}};
...
function renderSuggestionsList(){const c=document.getElementById('suggestionsAdminList');c.innerHTML='';if(allSuggestions.length===0){c.innerHTML='<p style="text-align:center;color:#666;padding:30px;">Nenhum item.</p>';return;}allSuggestions.forEach(s=>{const d=document.createElement('div');d.className='suggestion-box';const date=s.timestamp?new Date(s.timestamp).toLocaleDateString('pt-BR',{day:'2-digit',month:'2-digit',year:'2-digit',hour:'2-digit',minute:'2-digit'}):'?';const cb=suggestionsSelectMode?`<input type="checkbox" class="suggestion-checkbox" data-id="${s.id}" ${selectedSuggestionIds.has(s.id)?'checked':''}>`:'';const del=!suggestionsSelectMode?`<button class="btn-danger" style="font-size:10px;padding:4px 8px;" onclick="deleteSingleSuggestion('${s.id}')">🗑️</button>`:'';const typeBadge=s.type==='bug'?'<span style="background:#c62828;color:#fff;font-size:9px;padding:2px 6px;border-radius:4px;font-weight:800;">🐛 BUG</span>':'<span style="background:#1565c0;color:#fff;font-size:9px;padding:2px 6px;border-radius:4px;font-weight:800;">💡 SUGESTÃO</span>';d.innerHTML=`<div class="sg-header"><div style="display:flex;align-items:center;gap:8px;">${cb}<div><div class="sg-user">${s.userName||'Anônimo'} ${typeBadge}</div><div class="sg-email">${s.userEmail||''}</div></div></div><div style="display:flex;align-items:center;gap:6px;"><span class="sg-date">${date}</span>${del}</div></div><div class="sg-text">${s.text}</div>`;if(suggestionsSelectMode){const chk=d.querySelector('.suggestion-checkbox');if(chk)chk.onchange=()=>{if(chk.checked)selectedSuggestionIds.add(s.id);else selectedSuggestionIds.delete(s.id);updateDeleteSelectedBtn();};}c.appendChild(d);});}
function updateDeleteSelectedBtn(){const b=document.getElementById('btnDeleteSelectedSuggestions');b.textContent=selectedSuggestionIds.size>0?`🗑️ Apagar ${selectedSuggestionIds.size}`:'🗑️ Apagar';}
...
async function loadStorageInfo(){const c=document.getElementById('storageContent');c.innerHTML='<p style="text-align:center;color:#aaa;">⏳ Calculando...</p>';try{const snap=await get(ref(rtdb));const rd=snap.exists()?snap.val():{};const tb=estimateJsonBytes(rd),cb=rd.catalog?estimateJsonBytes(rd.catalog):0,ub=rd.users?estimateJsonBytes(rd.users):0,sb=rd.suggestions?estimateJsonBytes(rd.suggestions):0;const ob=Math.max(0,tb-cb-ub-sb),cc=rd.catalog?Object.keys(rd.catalog).length:0,uc=rd.users?Object.keys(rd.users).length:0,scc=rd.suggestions?Object.keys(rd.suggestions).length:0;const pct=Math.min(100,(tb/FIREBASE_RTDB_FREE_LIMIT_BYTES)*100);let bc='#4caf50';if(pct>70)bc='#ff9800';if(pct>90)bc='#f44336';c.innerHTML=`<div style="margin-bottom:18px;"><div style="display:flex;justify-content:space-between;margin-bottom:8px;"><span style="font-size:13px;font-weight:800;">Uso Total</span><span style="font-size:13px;font-weight:800;color:${bc};">${pct.toFixed(2)}%</span></div><div class="storage-bar-outer"><div class="storage-bar-inner" style="width:${pct}%;background:${bc};"></div></div><div class="storage-info"><span>${formatBytes(tb)} usado</span><span>${formatBytes(FIREBASE_RTDB_FREE_LIMIT_BYTES)} limite</span></div></div><h4 style="font-size:12px;font-weight:800;margin-bottom:10px;color:var(--primary-color);">📊 Detalhamento</h4><div class="storage-detail-item"><span class="storage-label">🎬 Catálogo (${cc})</span><span class="storage-value">${formatBytes(cb)}</span></div><div class="storage-detail-item"><span class="storage-label">👤 Usuários (${uc})</span><span class="storage-value">${formatBytes(ub)}</span></div><div class="storage-detail-item"><span class="storage-label">💡 Sugestões (${scc})</span><span class="storage-value">${formatBytes(sb)}</span></div><div class="storage-detail-item"><span class="storage-label">📁 Outros</span><span class="storage-value">${formatBytes(ob)}</span></div><div class="storage-detail-item" style="border-top:1px solid rgba(255,255,255,0.1);padding-top:10px;"><span class="storage-label" style="font-weight:800;color:#fff;">TOTAL</span><span class="storage-value" style="color:${bc};">${formatBytes(tb)}</span></div>`;}catch(e){c.innerHTML=`<p style="color:#ff5252;">❌ ${e.message}</p>`;}}

// ========== PROFILE ==========
document.getElementById('btnSaveProfile').onclick=async()=>{const user=auth.currentUser;if(!user)return;const themeColor=document.getElementById('themeColorPicker').value;if(!isTVDevice())applyUserTheme(themeColor);const n=document.getElementById('profileNameInput').value.trim(),b=document.getElementById('profileBioInput').value.trim(),fg=document.getElementById('profileFavGenreInput').value;const ph=document.getElementById('profilePhotoPreview').src,bn=document.getElementById('profileBannerPreview').src;const data={name:n||'',bio:b||'',favGenre:fg||'Ação',photo:(ph&&!ph.includes('window.location'))?ph:'',banner:(bn&&!bn.includes('window.location'))?bn:'',themeColor:themeColor||'#e50914'};try{await set(ref(rtdb,"users/"+user.uid),data);if(n)setUserCache('name',n);if(b)setUserCache('bio',b);if(fg)setUserCache('fav_genre',fg);if(data.photo)setUserCache('avatar',data.photo);if(data.banner)setUserCache('banner',data.banner);setUserCache('theme_color',themeColor);updateUserAvatarUI(data);showMsg('Perfil salvo!','success');document.getElementById('profileModal').classList.add('hidden');}catch(e){showMsg('Erro: '+e.message,'error');}};
...
function resetProfileUI(){document.getElementById('avatarText').innerText='U';document.getElementById('avatarImg').classList.add('hidden');document.getElementById('avatarText').classList.remove('hidden');document.getElementById('profileAvatarBigText').innerText='U';document.getElementById('profileAvatarBigImg').classList.add('hidden');document.getElementById('profileAvatarBigText').classList.remove('hidden');document.getElementById('profileNameDisplay').innerText='Usuário';document.getElementById('profileNameInput').value='';document.getElementById('profileBioDisplay').innerText='"Maratonando!"';document.getElementById('profileBioInput').value='';document.getElementById('profileFavGenreInput').value='Ação';document.getElementById('profileEmailDisplay').innerText='';document.getElementById('profileBannerImg').src='https://via.placeholder.com/600x200?text=Banner';document.getElementById('profilePhotoPreview').classList.add('hidden');document.getElementById('profileBannerPreview').classList.add('hidden');document.getElementById('sidebarUserName').innerText='Usuário';document.getElementById('sidebarUserEmail').innerText='email@exemplo.com';document.getElementById('sidebarAvatar').innerHTML='<span>U</span>';document.getElementById('themeColorPicker').value='#e50914';}

function updateUserAvatarUI(data={}){const un=data.name||getUserCache('name')||'';const ub=data.bio||getUserCache('bio')||'';const ug=data.favGenre||getUserCache('fav_genre')||'';const av=data.photo||getUserCache('avatar')||'';const bn=data.banner||getUserCache('banner')||'';const user=auth.currentUser;const letter=un?un.charAt(0).toUpperCase():(user?user.email.charAt(0).toUpperCase():'U');if(un){document.getElementById('profileNameDisplay').innerText=un;document.getElementById('profileNameInput').value=un;document.getElementById('sidebarUserName').innerText=un;}else if(user){document.getElementById('sidebarUserName').innerText=user.email.split('@')[0];document.getElementById('profileNameDisplay').innerText=user.email.split('@')[0];}if(user)document.getElementById('sidebarUserEmail').innerText=user.email;if(ub){document.getElementById('profileBioDisplay').innerText=`"${ub}"`;document.getElementById('profileBioInput').value=ub;}if(ug)document.getElementById('profileFavGenreInput').value=ug;if(bn){document.getElementById('profileBannerImg').src=bn;document.getElementById('profileBannerPreview').src=bn;document.getElementById('profileBannerPreview').classList.remove('hidden');}else{document.getElementById('profileBannerImg').src='https://via.placeholder.com/600x200?text=Banner';document.getElementById('profileBannerPreview').classList.add('hidden');}const sa=document.getElementById('sidebarAvatar');sa.innerHTML='';if(av){document.getElementById('avatarImg').src=av;document.getElementById('avatarImg').classList.remove('hidden');document.getElementById('avatarText').classList.add('hidden');document.getElementById('profileAvatarBigImg').src=av;document.getElementById('profileAvatarBigImg').classList.remove('hidden');document.getElementById('profileAvatarBigText').classList.add('hidden');document.getElementById('profilePhotoPreview').src=av;document.getElementById('profilePhotoPreview').classList.remove('hidden');const img=document.createElement('img');img.src=av;sa.appendChild(img);}else{document.getElementById('avatarText').innerText=letter;document.getElementById('avatarImg').classList.add('hidden');document.getElementById('avatarText').classList.remove('hidden');document.getElementById('profileAvatarBigText').innerText=letter;document.getElementById('profileAvatarBigImg').classList.add('hidden');document.getElementById('profileAvatarBigText').classList.remove('hidden');document.getElementById('profilePhotoPreview').classList.add('hidden');sa.innerHTML=`<span>${letter}</span>`;}}

async function handleLogout(){try{currentUserUid=null;await signOut(auth);resetProfileUI();document.documentElement.style.setProperty('--primary-color','#e50914');document.getElementById('profileModal').classList.add('hidden');closeSidebar();activeItem=null;mediaCatalog=[];isAdmin=false;showMsg('Saiu!','success');}catch(e){showMsg('Erro: '+e.message,'error');}}
document.getElementById('btnLogout').onclick=handleLogout;

// ========== AUTH ==========
document.getElementById('toggleAuthMode').onclick=()=>{isSignUpMode=!isSignUpMode;document.getElementById('authSubtitle').innerText=isSignUpMode?'Crie sua conta':'Entre na sua conta';document.getElementById('btnAuthSubmit').innerText=isSignUpMode?'Criar Conta':'Entrar na Conta';document.getElementById('toggleAuthMode').innerText=isSignUpMode?'Já tem conta? Entrar':'Não tem conta? Crie agora';document.getElementById('authEmail').value='';document.getElementById('authPassword').value='';};
document.getElementById('authForm').onsubmit=async(e)=>{e.preventDefault();if(authProcessing)return;authProcessing=true;const sb=document.getElementById('btnAuthSubmit');const ot=sb.innerText;sb.innerText='⏳ Aguarde...';sb.disabled=true;const email=document.getElementById('authEmail').value.trim().toLowerCase();const pass=document.getElementById('authPassword').value;if(!email||!pass){showMsg('Preencha tudo!','error');resetAuthBtn(sb,ot);return;}if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){showMsg('E-mail inválido!','error');resetAuthBtn(sb,ot);return;}if(pass.length<6){showMsg('Senha: mínimo 6!','error');resetAuthBtn(sb,ot);return;}try{if(isSignUpMode){try{const methods=await fetchSignInMethodsForEmail(auth,email);if(methods&&methods.length>0){showMsg('E-mail já cadastrado!','error');resetAuthBtn(sb,ot);return;}}catch(chk){}const cred=await createUserWithEmailAndPassword(auth,email,pass);await set(ref(rtdb,"users/"+cred.user.uid),{name:email.split('@')[0],bio:'',favGenre:'Ação',photo:'',banner:'',themeColor:'#e50914',createdAt:Date.now()});showMsg('✅ Conta criada!','success');document.getElementById('authOverlay').classList.add('hidden');}else{await signInWithEmailAndPassword(auth,email,pass);showMsg('✅ Bem-vindo(a)!','success');document.getElementById('authOverlay').classList.add('hidden');}}catch(err){showMsg(translateAuthError(err.code),'error');}resetAuthBtn(sb,ot);};
function resetAuthBtn(b,t){authProcessing=false;b.innerText=t;b.disabled=false;}
...
onAuthStateChanged(auth,(user)=>{
    if(user){
        ...
        if(isAdmin){document.getElementById('profileAdminBadge').classList.remove('hidden');document.getElementById('sidebarAdminItem').classList.remove('hidden');document.getElementById('sidebarCreatorItem').classList.remove('hidden');document.getElementById('sidebarSuggestionsAdminItem').classList.remove('hidden');document.getElementById('sidebarStorageItem').classList.remove('hidden');}
        else{document.getElementById('profileAdminBadge').classList.add('hidden');document.getElementById('sidebarAdminItem').classList.add('hidden');document.getElementById('sidebarCreatorItem').classList.add('hidden');document.getElementById('sidebarSuggestionsAdminItem').classList.add('hidden');document.getElementById('sidebarStorageItem').classList.add('hidden');}
        ...
        document.getElementById('sidebarAdminItem').classList.add('hidden');document.getElementById('sidebarCreatorItem').classList.add('hidden');document.getElementById('sidebarSuggestionsAdminItem').classList.add('hidden');document.getElementById('sidebarStorageItem').classList.add('hidden');document.getElementById('profileAdminBadge').classList.add('hidden');
        document.getElementById('authEmail').value='';document.getElementById('authPassword').value='';
        isSignUpMode=false;document.getElementById('authSubtitle').innerText='Entre na sua conta para continuar';document.getElementById('btnAuthSubmit').innerText='Entrar na Conta';document.getElementById('toggleAuthMode').innerText='Não tem uma conta? Crie agora';
    }
});
