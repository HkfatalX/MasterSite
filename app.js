...
document.querySelectorAll('.sidebar-item').forEach(item => {
    item.onclick = () => {
        ...
        else if (a === 'suggestions') { document.getElementById('suggestionModal').classList.remove('hidden'); document.body.style.overflow='hidden'; }
        else if (a === 'profile') { document.getElementById('profileModal').classList.remove('hidden'); document.body.style.overflow='hidden'; }
        else if (a === 'admin') { renderAdminCatalogList(); document.getElementById('adminModal').classList.remove('hidden'); document.body.style.overflow='hidden'; }
        else if (a === 'creator') { openCreator(); }
        else if (a === 'suggestionsAdmin') { loadSuggestionsAdmin(); document.getElementById('suggestionsAdminModal').classList.remove('hidden'); document.body.style.overflow='hidden'; }
        else if (a === 'storage') { loadStorageInfo(); document.getElementById('storageModal').classList.remove('hidden'); document.body.style.overflow='hidden'; }
        ...
        reader.onload = (ev) => {
            ...
            tmp.onload = () => {
                ...
                cropperInstance = new Cropper(document.getElementById('cropperImage'), {
                    ...
                    crop() {
                        const cv = cropperInstance.getCroppedCanvas({ width: 800, imageSmoothingQuality: 'high' });
                        if (cv) {
                            const u = cv.toDataURL('image/jpeg', 0.90);
...
searchInput.addEventListener('keydown', (e) => {
    ...
    else if (e.key === 'Enter') { e.preventDefault(); const f = searchDropdown.querySelector('.search-result-item'); if (f) f.click(); }
    else if (e.key === 'ArrowDown') { e.preventDefault(); const f = searchDropdown.querySelector('.search-result-item'); if (f) f.focus(); }
});
searchDropdown.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'ArrowDown') { e.preventDefault(); if (items[ci + 1]) items[ci + 1].focus(); }
    else if (e.key === 'ArrowUp') { e.preventDefault(); if (ci <= 0) searchInput.focus(); else items[ci - 1].focus(); }
    else if (e.key === 'Enter') { e.preventDefault(); if (document.activeElement?.classList.contains('search-result-item')) document.activeElement.click(); }
...
function smartSearch(q) {
    const nq = normalizeText(q); if (!nq) return []; const sc = [];
    mediaCatalog.forEach(i => {
        const t = normalizeText(i.title); if (!t) return;
        let s = 0;
        if (t.startsWith(nq)) s = 1000 - t.length;
        else { let ws = false; for (const w of t.split(/\s+/)) if (w.startsWith(nq)) { ws = true; break; } if (ws) s = 500 - t.length; else if (t.includes(nq)) s = 100 - t.length; }
        if (s > 0) sc.push({ item: i, score: s });
    });
    sc.sort((a, b) => b.score - a.score); return sc.slice(0, 8).map(s => s.item);
}
...
function renderSearchDropdown(query) {
    ...
    if (results.length === 0) { searchDropdown.innerHTML = `<div class="search-no-results">🔍 Nada encontrado</div>`; }
        ...
        results.forEach(item => {
            ...
            let gt = '';
            if (Array.isArray(item.genres) && item.genres.length > 0) gt = item.genres.slice(0, 2).join(', ');
            else if (item.category) gt = item.category;
...
document.getElementById('mediaType').onchange = (e) => {
    ...
    if (t === 'movie') {
        ...
        document.getElementById('mediaDuration').placeholder = "Ex: 2h 10m";
    } else {
        ...
        document.getElementById('mediaDuration').placeholder = "Ex: 45m/ep";
...
function renderSeasonsBuilder() {
    ...
    seasonsBuilder.forEach((s, si) => {
        ...
        (s.episodes || []).forEach((ep, ei) => {
            ...
            ed.style.cssText = "background:#111;border:1px solid #282828;border-radius:6px;padding:10px;margin-top:10px;";
...
function renderApp() {
    ...
    mediaCatalog.forEach(item => {
        ...
        card.tabIndex = 0; // Foco da TV
        
        const ps = getPosterUrl(item), gs = getItemGenres(item), tg = gs.length > 0 ? gs[0] : (item.type === 'movie' ? 'Filme' : 'Série');
        if (ps) {
            card.innerHTML = `<img class="media-card-poster" src="${ps}" alt="${item.title}" loading="lazy" onerror="this.style.display='none';this.parentElement.querySelector('.media-card-fallback-hidden').style.display='flex';"><div class="media-card-poster-fallback media-card-fallback-hidden" style="display:none;position:absolute;inset:0;">🎬</div><div class="media-card-overlay"><span class="media-card-tag">${tg}</span><div class="media-card-title">${item.title}</div></div>`;
        } else {
            card.innerHTML = `<div class="media-card-poster-fallback">🎬</div><div class="media-card-overlay"><span class="media-card-tag">${tg}</span><div class="media-card-title">${item.title}</div></div>`;
        }
        
        card.onclick = () => { window.location.hash = `#/midia/${item.id}`; openDetails(item); };
        card.onkeydown = (e) => { if(e.key === 'Enter') card.click(); }; // Controle de TV
...
function openDetails(item) {
    ...
    if (item.type === 'movie') {
        ...
        setTimeout(() => btnPlay.focus(), 100); // TV Focus
    } else {
        ...
        (item.seasons || []).forEach((s, idx) => {
            ...
            tab.tabIndex = 0; // TV Focus
            tab.onclick = () => {
                document.querySelectorAll('.season-tab').forEach(t => t.classList.remove('active'));
                tab.classList.add('active');
                if (s.seasonCoverUrl?.trim()) ba.style.backgroundImage = `url('${s.seasonCoverUrl}')`; else ba.style.backgroundImage = bd ? `url('${bd}')` : ``;
        ...
        document.getElementById('detailsModal').classList.remove('hidden');
        document.body.style.overflow = 'hidden'; // Trava o fundo
        setTimeout(() => { const firstTab = document.querySelector('.season-tab'); if(firstTab) firstTab.focus(); }, 100); // TV Focus
    }
}

// CORREÇÃO: Hierarquia de Imagens de Fallback
function renderEpisodesList(eps, si, sd, ser, resumeEpIdx) {
    const c = document.getElementById('episodesListContainer'); c.innerHTML = "";
    
    // Se a temporada não tiver foto, pega a da série (backdrop ou poster)
    const fallbackImage = (sd && sd.seasonCoverUrl && sd.seasonCoverUrl.trim() !== '' && !sd.seasonCoverUrl.includes('window.location')) 
        ? sd.seasonCoverUrl 
    ...
    eps.forEach((ep, idx) => {
        ...
        // Se o episódio não tiver thumb, usa o fallback
        const th = (ep.thumbUrl && ep.thumbUrl.trim() !== '' && !ep.thumbUrl.includes('window.location')) ? ep.thumbUrl : fallbackImage;
        ...
        d.onkeydown = (e) => { if(e.key === 'Enter') d.click(); }; // TV Focus

        d.innerHTML = `
            <div class="episode-thumb"><img src="${th}" onerror="this.src='${fallbackImage}'"></div>
            <div style="flex:1;">
...
// CORREÇÃO DO BUG DO BLUR: Removida a linha que fechava o fullscreen sozinho
...
document.getElementById('btnToggleFullscreen').onclick = (e) => {
    e.stopPropagation();
    if (!isInFullscreen()) { playerBox.requestFullscreen?.() ?? playerBox.webkitRequestFullscreen?.() ?? playerBox.msRequestFullscreen?.(); }
...
function playVideo(rawUrl, mainTitle, subTitle) {
    ...
    // CORREÇÃO: Sandbox para matar os redirecionamentos de anúncios!
    ...
    // Deixa o iframe no tamanho absoluto
    iframe.style.cssText = 'width:100vw;height:100vh;position:absolute;top:0;left:0;border:none;';
    ...
    updateFullscreenButtonText(); forceShowControls(); showPlayerControls();
...
document.getElementById('mediaForm').onsubmit = async (e) => {
    ...
    if (type === 'movie') payload.videoUrl = document.getElementById('movieVideoUrl').value.trim();
        ...
        for (let si = 0; si < seasonsBuilder.length; si++) {
            ...
            for (let ei = 0; ei < (s.episodes || []).length; ei++) {
                ...
                let et = document.getElementById(`epThumbPrev_${si}_${ei}`)?.src || (ep.thumbUrl || '');
            ...
            us.push({ seasonNumber: s.seasonNumber || si + 1, seasonCoverUrl: sc, episodes: ue });
        }
        payload.seasons = us;
    }
    ...
    try {
        ...
        document.getElementById('creatorModal').classList.add('hidden'); document.body.style.overflow = '';
...
window.editMedia = (id) => {
    ...
    ts.value = i.type; ts.disabled = true; ts.style.cursor = 'not-allowed'; ts.style.opacity = '0.6'; ts.dispatchEvent(new Event('change'));
...
document.getElementById('btnCloseAdmin').onclick = () => { document.getElementById('adminModal').classList.add('hidden'); document.body.style.overflow = ''; };
...
function openCreator() {
    ...
    document.getElementById('seasonsList').innerHTML = ""; addSeason();
    document.getElementById('creatorModal').classList.remove('hidden'); document.body.style.overflow = 'hidden';
}

// ========== SUGGESTIONS ==========
document.getElementById('btnCloseSuggestion').onclick = () => { document.getElementById('suggestionModal').classList.add('hidden'); document.body.style.overflow = ''; };
document.getElementById('btnSendSuggestion').onclick = async () => {
    ...
    const text = document.getElementById('suggestionText').value.trim(); const type = document.getElementById('suggestionType').value;
    ...
    try {
        ...
        document.getElementById('suggestionText').value = ''; showMsg(type === 'bug' ? 'Bug reportado! 🐛' : 'Sugestão enviada! 💡', 'success');
        document.getElementById('suggestionModal').classList.add('hidden'); document.body.style.overflow = '';
    } catch (e) { showMsg('Erro: ' + e.message, 'error'); }
};

// ========== SUGGESTIONS ADMIN ==========
document.getElementById('btnCloseSuggestionsAdmin').onclick = () => { exitSelectMode(); document.getElementById('suggestionsAdminModal').classList.add('hidden'); document.body.style.overflow = ''; };
...
// ========== STORAGE ==========
document.getElementById('btnCloseStorage').onclick = () => { document.getElementById('storageModal').classList.add('hidden'); document.body.style.overflow = ''; };
...
async function handleLogout() { try { currentUserUid = null; await signOut(auth); resetProfileUI(); document.documentElement.style.setProperty('--primary-color', '#e50914'); document.getElementById('profileModal').classList.add('hidden'); closeSidebar(); document.body.style.overflow = ''; activeItem = null; mediaCatalog = []; isAdmin = false; showMsg('Saiu!', 'success'); } catch (e) { showMsg('Erro: ' + e.message, 'error'); } }
...
onAuthStateChanged(auth, (user) => {
    if (user) {
        ...
        resetProfileUI();
        document.getElementById('authOverlay').classList.add('hidden'); document.body.style.overflow = '';
        ...
        document.getElementById('authOverlay').classList.remove('hidden'); document.body.style.overflow = 'hidden';
