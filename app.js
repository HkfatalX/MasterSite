...
import {
    getDatabase, ref, get, set, push, remove, runTransaction
...
const GENRES = [
    "Lançamento","Ação","Aventura","Comédia","Drama","Terror","Suspense",
    "Romance","Ficção Científica","Fantasia","Animação","Documentário",
    "Crime","Família","Anime","Dorama","Musical","Mistério","Guerra",
    "Histórico","Faroeste","Super-Heróis"
];
const DB_LIMIT = 1073741824;
const CONTINUE_EXPIRE = 7*24*60*60*1000;
const DEV_KEY = "mfx_device_id";
const HEARTBEAT = 60000;
const HIDE_CONTROLS_MS = 6000;
const HIDE_CURSOR_MS = 5000;
...
const fmtBytes = b => {if(!b)return"0 B";const k=1024,s=["B","KB","MB","GB"],i=Math.floor(Math.log(b)/Math.log(k));return`${(b/Math.pow(k,i)).toFixed(2)} ${s[i]}`;};
const jsonSize = o => {try{return new Blob([JSON.stringify(o)]).size}catch{return JSON.stringify(o).length*2}};
...
let catalog=[],selCat="Todos",signUpMode=false,selGenres=[],
    cropInst=null,cropTarget=null,prevModal=null,
    isAdm=false,authBusy=false,curUid=null,
    allSugg=[],suggSelMode=false,suggSelIds=new Set(),
    dailyViews={},playingId=null,viewTracked=false;

// Player
let vidUrl="",sources=[],srcIdx=0,pMeta={main:"",sub:""},
    errTimer=null,stretch=false,
    hideTimer=null,cursorTimer=null,ctrlLocked=false;
...
const ADAPTERS={
    mixdrop:{name:"MixDrop",color:"#00c6ff",icon:"🎬",ms:18000,
        fmt(u){let f=u.trim();if(f.includes("/f/"))f=f.replace("/f/","/e/");else if(!f.includes("/e/")){const m=f.match(/mixdrop\.[a-z]+\/(?:e\/|f\/)?([a-zA-Z0-9]+)/);if(m?.[1])f=`https://mixdrop.ag/e/${m[1]}`;}return f;},
        mk(u){return mkIframe(u);}
    },
    streamtape:{name:"StreamTape",color:"#f5a623",icon:"📼",ms:18000,
        fmt(u){let f=u.trim();if(f.includes("/v/"))f=f.replace("/v/","/e/");else if(!f.includes("/e/")){const m=f.match(/streamtape\.[a-z]+\/(?:v|e)\/([a-zA-Z0-9]+)/);if(m?.[1])f=`https://streamtape.com/e/${m[1]}`;}return f;},
        mk(u){return mkIframe(u);}
    },
    filemoon:{name:"FileMoon",color:"#8e44ad",icon:"🌙",ms:18000,
        fmt(u){let f=u.trim();if(!f.includes("/e/")&&!f.includes("/embed")){const m=f.match(/filemoon\.[a-z]+\/(?:d\/|e\/)?([a-zA-Z0-9]+)/);if(m?.[1])f=`https://filemoon.sx/e/${m[1]}`;}return f;},
        mk(u){return mkIframe(u);}
    },
    voe:{name:"VOE",color:"#ff4757",icon:"⚡",ms:18000,fmt(u){return u.trim();},mk(u){return mkIframe(u);}},
    doodstream:{name:"DoodStream",color:"#f39c12",icon:"🔥",ms:22000,
        fmt(u){let f=u.trim();if(f.includes("/d/"))f=f.replace("/d/","/e/");else if(!f.includes("/e/")){const m=f.match(/(?:dood[a-z0-9]*\.[a-z]+)\/(?:d\/|e\/)?([a-zA-Z0-9]+)/);if(m?.[1]){const dm=f.match(/(https?:\/\/[^/]+)/);f=`${dm?.[1]||"https://dood.li"}/e/${m[1]}`;}}return f;},
        mk(u){return mkIframe(u);}
    },
    mp4upload:{name:"Mp4Upload",color:"#3498db",icon:"📤",ms:18000,
        fmt(u){let f=u.trim();if(!f.includes("/embed")){const m=f.match(/mp4upload\.com\/(?:embed-)?([a-zA-Z0-9]+)/);if(m?.[1])f=`https://www.mp4upload.com/embed-${m[1]}.html`;}return f;},
        mk(u){return mkIframe(u);}
    },
    okru:{name:"OK.ru",color:"#ee8208",icon:"📡",ms:18000,
        fmt(u){const m=u.match(/ok\.ru\/(?:video|videoembed)\/(\d+)/);return m?.[1]?`https://ok.ru/videoembed/${m[1]}`:u.trim();},
        mk(u){return mkIframe(u);}
    },
    drive:{name:"Google Drive",color:"#4285f4",icon:"💾",ms:16000,
        fmt(u){let f=u.trim();const m=f.match(/\/d\/([a-zA-Z0-9_-]+)/);if(m?.[1])return`https://drive.google.com/file/d/${m[1]}/preview`;if(f.includes("open?id=")){const m2=f.match(/id=([a-zA-Z0-9_-]+)/);if(m2?.[1])return`https://drive.google.com/file/d/${m2[1]}/preview`;}return f;},
        mk(u){return mkIframe(u);}
    },
    yourupload:{name:"YourUpload",color:"#27ae60",icon:"⬆️",ms:18000,
        fmt(u){let f=u.trim();if(!f.includes("/embed/")){const m=f.match(/yourupload\.com\/(?:watch|embed)\/([a-zA-Z0-9]+)/);if(m?.[1])f=`https://www.yourupload.com/embed/${m[1]}`;}return f;},
        mk(u){return mkIframe(u);}
    },
    youtube:{name:"YouTube",color:"#ff0000",icon:"▶️",ms:12000,
        fmt(u){const m=u.trim().match(/(?:youtube\.com\/(?:watch\?v=|embed\/|v\/)|youtu\.be\/)([a-zA-Z0-9_-]{11})/);return m?.[1]?`https://www.youtube.com/embed/${m[1]}?autoplay=1&rel=0`:u.trim();},
        mk(u){return mkIframe(u);}
    },
    vimeo:{name:"Vimeo",color:"#17d5ff",icon:"🎥",ms:12000,
        fmt(u){const m=u.trim().match(/vimeo\.com\/(?:video\/)?(\d+)/);return m?.[1]?`https://player.vimeo.com/video/${m[1]}?autoplay=1`:u.trim();},
        mk(u){return mkIframe(u);}
    },
    direct:{name:"Direto",color:"#4caf50",icon:"📹",ms:12000,
        fmt(u){return u.trim();},
        mk(u){const v=document.createElement("video");v.src=u;v.controls=true;v.autoplay=true;v.playsInline=true;v.setAttribute("webkit-playsinline","true");return v;}
    },
    other:{name:"Outro",color:"#888",icon:"🔗",ms:18000,fmt(u){return u.trim();},mk(u){return mkIframe(u);}}
};
function adapter(t){return ADAPTERS[t]||ADAPTERS.other;}
...
function typeLabel(t){return adapter(t).name;}
function normSources(item){if(Array.isArray(item.videoSources)&&item.videoSources.length)return item.videoSources;if(item.videoUrl?.trim()){const t=detectType(item.videoUrl);return[{type:t,label:typeLabel(t),url:item.videoUrl.trim()}];}return[];}
function normEpSources(ep){if(Array.isArray(ep.videoSources)&&ep.videoSources.length)return ep.videoSources;if(ep.videoUrl?.trim()){const t=detectType(ep.videoUrl);return[{type:t,label:typeLabel(t),url:ep.videoUrl.trim()}];}return[];}
...
qa(".sidebar-item").forEach(it=>{
    const act=()=>{const a=it.dataset.nav;qa(".sidebar-item").forEach(i=>i.classList.remove("active"));if(["home","movies","series","continue"].includes(a))it.classList.add("active");closeSB();
    const nav={home(){selCat="Todos";updateChips();render();scrollTo({top:0,behavior:"smooth"});},movies(){selCat="Filmes";updateChips();render();scrollTo({top:0,behavior:"smooth"});},series(){selCat="Séries";updateChips();render();scrollTo({top:0,behavior:"smooth"});},continue(){const r=$("continueRow");r&&!r.classList.contains("hidden")?r.scrollIntoView({behavior:"smooth"}):showMsg("Nada em andamento!","info");},suggestions(){openModal("suggestionModal");},profile(){openProfileModal();},admin(){renderAdmin();openModal("adminModal");},creator(){openCreator();},suggestionsAdmin(){loadSuggAdmin();openModal("suggestionsAdminModal");},storage(){loadStorage();openModal("storageModal");},logout(){if(confirm("Sair?"))doLogout();}};nav[a]?.();};
    it.addEventListener("click",act);it.addEventListener("keydown",e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();act();}});
});
...
function smartSearch(q){const nq=norm(q);if(!nq)return[];const r=[];catalog.forEach(i=>{const t=norm(i.title);if(!t)return;let s=0;if(t===nq)s=10000;else if(t.startsWith(nq))s=1000-t.length;else if(t.includes(nq))s=100-t.length;else{const hits=nq.split(" ").filter(Boolean).filter(w=>t.includes(w)).length;if(hits)s=hits*20;}if(s>0)r.push({item:i,score:s});});r.sort((a,b)=>b.score-a.score);return r.slice(0,10).map(x=>x.item);}
function hiMatch(t,q){const nt=norm(t),nq=norm(q),i=nt.indexOf(nq);if(i===-1)return t;return t.slice(0,i)+"<mark>"+t.slice(i,i+q.length)+"</mark>"+t.slice(i+q.length);}
function renderSearch(query){if(!sDrop)return;const res=smartSearch(query);sDrop.innerHTML="";if(!res.length){sDrop.innerHTML=`<div class="search-no-results"><span class="search-empty-icon">🔍</span><div>Nenhum resultado</div></div>`;}else{sDrop.innerHTML=`<div class="search-results-header">🔍 ${res.length} resultado(s)</div>`;res.forEach(it=>{const d=document.createElement("div");d.className="search-result-item";d.tabIndex=0;const p=poster(it),tl=it.type==="movie"?"Filme":"Série",gs=genres(it);d.innerHTML=`<img class="search-result-thumb" src="${p}" loading="lazy" onerror="this.style.display='none'"><div class="search-result-info"><div class="search-result-title">${hiMatch(it.title,query)}</div><div class="search-result-meta"><span class="type-badge">${tl}</span>${it.year?`<span>${it.year}</span>`:""}${gs.length?`<span>${gs.slice(0,2).join(", ")}</span>`:""}</div></div>`;const go=()=>{sDrop.classList.remove("visible");sIn.value="";sBox?.classList.remove("has-text");location.hash=`#/midia/${it.id}`;openDetails(it);};d.addEventListener("click",go);d.addEventListener("keydown",e=>{if(e.key==="Enter")go();});sDrop.appendChild(d);});}sDrop.classList.add("visible");}

// ═══════════════════════════════
// FEATURED
// ═══════════════════════════════
function sortViews(items){return[...items].sort((a,b)=>viewCount(b.id)-viewCount(a.id)||(parseInt(b.year||"0")||0)-(parseInt(a.year||"0")||0));}
function getFeatured(){if(!catalog.length)return[];if(selCat==="Todos"){const m=sortViews(catalog.filter(i=>i.type==="movie")).slice(0,2);const s=sortViews(catalog.filter(i=>i.type==="serie")).slice(0,2);return[...s,...m].sort((a,b)=>viewCount(b.id)-viewCount(a.id));}if(selCat==="Filmes")return sortViews(catalog.filter(i=>i.type==="movie")).slice(0,4);if(selCat==="Séries")return sortViews(catalog.filter(i=>i.type==="serie")).slice(0,4);return sortViews(catalog.filter(i=>matchCat(i,selCat))).slice(0,4);}
function renderFeatured(){const grid=$("featuredGrid"),hd=$("featuredHeading"),sub=$("featuredSub");if(!grid)return;if(selCat==="Todos"){hd.innerText="Top 4 de hoje";sub.innerText="2 séries + 2 filmes mais assistidos.";}else if(selCat==="Filmes"){hd.innerText="Filmes em alta";sub.innerText="Os mais assistidos hoje.";}else if(selCat==="Séries"){hd.innerText="Séries em alta";sub.innerText="As mais assistidas hoje.";}else{hd.innerText=selCat+" em alta";sub.innerText=`Os títulos mais assistidos de ${selCat}.`;}
const items=getFeatured();grid.innerHTML="";if(!items.length){grid.innerHTML=`<div class="featured-empty">Nenhum destaque disponível.</div>`;return;}
const tmpl=$("featuredCardTemplate");items.forEach((it,idx)=>{const n=tmpl.content.cloneNode(true);const card=n.querySelector(".featured-card");n.querySelector(".featured-card-bg").style.backgroundImage=`url('${backdrop(it)||poster(it)}')`;n.querySelector(".featured-rank").innerText=`#${idx+1}`;n.querySelector(".featured-card-badge").innerText=it.type==="movie"?"🎬 Filme":"📺 Série";n.querySelector(".featured-card-watch").innerText=`👁️ ${viewCount(it.id)} views`;n.querySelector(".featured-card-title").innerText=it.title;const gs=genres(it);n.querySelector(".featured-card-meta").innerHTML=`<span>${it.type==="movie"?"Filme":"Série"}</span>${it.year?`<span class="dot">•</span><span>${it.year}</span>`:""}${it.duration?`<span class="dot">•</span><span>${it.duration}</span>`:""}${gs.length?`<span class="dot">•</span><span>${gs.slice(0,2).join(", ")}</span>`:""}`;n.querySelector(".featured-card-desc").innerText=it.description||"";const go=()=>{location.hash=`#/midia/${it.id}`;openDetails(it);};card.addEventListener("click",go);card.addEventListener("keydown",e=>{if(e.key==="Enter")go();});n.querySelector(".featured-mini-btn.play").addEventListener("click",e=>{e.stopPropagation();if(it.type==="movie"){saveContinue(it);initiatePlay(normSources(it),it.title,"Filme",it.id);}else openDetails(it);});n.querySelector(".featured-mini-btn.info").addEventListener("click",e=>{e.stopPropagation();openDetails(it);});grid.appendChild(n);});}
...
// ═══════════════════════════════
function renderMovSrc(){const c=$("movieSourcesList");if(!c)return;c.innerHTML="";if(!creator.movieSrc.length){c.innerHTML=`<div class="source-empty-state">Nenhuma fonte. Clique + Adicionar.</div>`;return;}
const tmpl=$("videoSourceItemTemplate");creator.movieSrc.forEach((src,idx)=>{const cl=tmpl.content.cloneNode(true);const card=cl.querySelector(".source-item-card");const ts=card.querySelector(".source-type-select"),ti=card.querySelector(".source-title-input"),ui=card.querySelector(".source-url-input");card.querySelector(".source-item-badge").textContent=`Fonte ${idx+1}`;ts.value=src.type||"";ti.value=src.label||"";ui.value=src.url||"";
const used=creator.movieSrc.filter((_,i)=>i!==idx).map(s=>s.type).filter(Boolean);[...ts.options].forEach(o=>{if(o.value&&o.value!=="other"&&o.value!==src.type&&used.includes(o.value))o.disabled=true;});
ts.onchange=()=>{const nt=ts.value;if(nt&&nt!=="other"&&creator.movieSrc.some((s,i)=>i!==idx&&s.type===nt)){showMsg(`Já existe "${typeLabel(nt)}"!`);ts.value=src.type||"";return;}creator.movieSrc[idx].type=nt;if(!creator.movieSrc[idx].label&&nt)creator.movieSrc[idx].label=typeLabel(nt);renderMovSrc();};
ti.oninput=()=>creator.movieSrc[idx].label=ti.value;
ui.oninput=()=>{creator.movieSrc[idx].url=ui.value;if(!creator.movieSrc[idx].type&&ui.value){const d=detectType(ui.value);if(d!=="other"&&!creator.movieSrc.some((s,i)=>i!==idx&&s.type===d)){creator.movieSrc[idx].type=d;if(!creator.movieSrc[idx].label)creator.movieSrc[idx].label=typeLabel(d);renderMovSrc();}}};
card.querySelector("[data-action='remove-source']").onclick=()=>{creator.movieSrc.splice(idx,1);renderMovSrc();};
if(src.type){const a=adapter(src.type);const tag=document.createElement("div");tag.className="source-preview-tag";tag.textContent=`${a.icon} ${a.name}`;card.appendChild(tag);}
c.appendChild(card);});}
...
// Dados NÃO somem mais!
// ═══════════════════════════════
function syncInputs(){
    if(creator.type!=="serie")return;
    creator.seasons.forEach(s=>{
        (s.episodes||[]).forEach(ep=>{
            const tEl=q(`[data-ep-title="${ep.id}"]`);
            const dEl=q(`[data-ep-dur="${ep.id}"]`);
            if(tEl)ep.title=tEl.value;
            if(dEl)ep.duration=dEl.value;
            // Sync sources too
            (ep.sources||[]).forEach(src=>{
                const stEl=q(`[data-src-type="${src.id}"]`);
                const slEl=q(`[data-src-label="${src.id}"]`);
                const suEl=q(`[data-src-url="${src.id}"]`);
                if(stEl)src.type=stEl.value;
                if(slEl)src.label=slEl.value;
                if(suEl)src.url=suEl.value;
            });
        });
    });
}
...
function renderEpSrc(ep){
    const c=q(`[data-ep-sources="${ep.id}"]`);if(!c)return;c.innerHTML="";
    const sources=ep.sources||[];
    if(!sources.length){c.innerHTML=`<div class="source-empty-state" style="font-size:11px">Sem fontes.</div>`;return;}
    const tmpl=$("videoSourceItemTemplate");
    sources.forEach((src,si)=>{
        const cl=tmpl.content.cloneNode(true);const card=cl.querySelector(".source-item-card");
        const ts=card.querySelector(".source-type-select"),ti=card.querySelector(".source-title-input"),ui=card.querySelector(".source-url-input");
        card.querySelector(".source-item-badge").textContent=`Fonte ${si+1}`;
        ts.value=src.type||"";ti.value=src.label||"";ui.value=src.url||"";
        // Mark with data attrs for syncInputs
        ts.setAttribute("data-src-type",src.id);
        ti.setAttribute("data-src-label",src.id);
        ui.setAttribute("data-src-url",src.id);
        const used=sources.filter((_,i)=>i!==si).map(s=>s.type).filter(Boolean);
        [...ts.options].forEach(o=>{if(o.value&&o.value!=="other"&&o.value!==src.type&&used.includes(o.value))o.disabled=true;});
        ts.onchange=()=>{const nt=ts.value;if(nt&&nt!=="other"&&sources.some((s,i)=>i!==si&&s.type===nt)){showMsg("Tipo duplicado!");ts.value=src.type||"";return;}src.type=nt;if(!src.label&&nt)src.label=typeLabel(nt);renderEpSrc(ep);};
        ti.oninput=()=>src.label=ti.value;
        ui.oninput=()=>{src.url=ui.value;if(!src.type&&ui.value){const d=detectType(ui.value);if(d!=="other"&&!sources.some((s,i)=>i!==si&&s.type===d)){src.type=d;if(!src.label)src.label=typeLabel(d);renderEpSrc(ep);}}};
        card.querySelector("[data-action='remove-source']").onclick=()=>{sources.splice(si,1);renderEpSrc(ep);};
        c.appendChild(card);
    });
}

function renderEpList(season){
    const c=q(`[data-ep-list="${season.id}"]`);if(!c)return;c.innerHTML="";
    const eps=season.episodes||[];
    if(!eps.length){c.innerHTML=`<div class="empty-list-state" style="padding:14px"><span class="empty-icon" style="font-size:24px">🎬</span>Nenhum episódio</div>`;return;}
    eps.forEach((ep,ei)=>{
        const isOpen=uiSt.openE.has(ep.id);
        const hasSrc=ep.sources?.some(s=>s.url?.trim());
        const srcN=ep.sources?.filter(s=>s.url?.trim()).length||0;
        const acc=document.createElement("div");acc.className="episode-accordion"+(isOpen?" open":"");
        acc.innerHTML=`<div class="episode-accordion-header" data-toggle-ep="${ep.id}"><div class="episode-accordion-header-left"><span class="ep-number-badge">${ei+1}</span><div class="ep-info-mini"><div class="ep-info-mini-title">${ep.title||`Episódio ${ei+1}`}</div><div class="ep-info-mini-sub">${ep.duration?`⏱️ ${ep.duration}`:""}${hasSrc?`<span class="ok-badge">✓ ${srcN} fonte(s)</span>`:`<span class="warn-badge">⚠ sem fonte</span>`}</div></div></div><div class="episode-accordion-actions"><button type="button" class="season-mini-btn danger" data-rm-ep="${season.id}|${ep.id}" title="Remover">🗑️</button><span class="ep-toggle-icon">▶</span></div></div><div class="episode-body"><div class="input-group"><label>Título</label><input type="text" data-ep-title="${ep.id}" value="${(ep.title||"").replace(/"/g,"&quot;")}" autocomplete="off"></div><div class="input-group"><label>Duração</label><input type="text" data-ep-dur="${ep.id}" value="${(ep.duration||"").replace(/"/g,"&quot;")}" placeholder="Ex: 45m" autocomplete="off"></div><div class="input-group"><label>🖼️ Thumbnail</label><label class="file-upload-box" id="etb_${ep.id}"><span class="file-upload-label">📸 Escolher</span><input type="file" id="eti_${ep.id}" accept="image/*" class="hidden"><img id="etp_${ep.id}" src="${ep.thumbUrl||""}" class="file-preview-img ${ep.thumbUrl?"":"hidden"}"></label></div><div class="input-group"><label style="font-size:11px;font-weight:800;color:var(--primary);text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px">🎞️ Fontes</label><div data-ep-sources="${ep.id}"></div><button type="button" class="add-btn-block" data-add-src="${ep.id}" style="margin-top:10px;font-size:11px;padding:10px">+ Adicionar Fonte</button></div></div>`;
        c.appendChild(acc);
        renderEpSrc(ep);
        q(`[data-toggle-ep="${ep.id}"]`,acc).onclick=e=>{if(e.target.closest("button"))return;syncInputs();uiSt.openE.has(ep.id)?uiSt.openE.delete(ep.id):uiSt.openE.add(ep.id);renderSeasons();};
        q(`[data-rm-ep="${season.id}|${ep.id}"]`,acc).onclick=e=>{e.stopPropagation();if(!confirm("Remover episódio?"))return;syncInputs();const idx=season.episodes.findIndex(x=>x.id===ep.id);if(idx>=0){season.episodes.splice(idx,1);uiSt.openE.delete(ep.id);renderSeasons();}};
        q(`[data-add-src="${ep.id}"]`,acc).onclick=()=>{syncInputs();ep.sources=ep.sources||[];ep.sources.push({id:uid(),type:"",label:"",url:""});renderSeasons();};
        // Live update
        q(`[data-ep-title="${ep.id}"]`,acc).oninput=e=>ep.title=e.target.value;
        q(`[data-ep-dur="${ep.id}"]`,acc).oninput=e=>ep.duration=e.target.value;
        $(`etb_${ep.id}`)?.addEventListener("click",ev=>{if(ev.target.closest("input"))return;ev.preventDefault();openCrop(`eti_${ep.id}`,`etp_${ep.id}`,16/9,u=>ep.thumbUrl=u);});
    });
}

function renderSeasons(){
    const c=$("seasonsList");if(!c)return;c.innerHTML="";
    if(!creator.seasons.length){c.innerHTML=`<div class="empty-list-state"><span class="empty-icon">📺</span>Nenhuma temporada.</div>`;return;}
    creator.seasons.forEach((s,si)=>{
        const isOpen=uiSt.openS.has(s.id);const epN=(s.episodes||[]).length;const hasC=(s.episodes||[]).some(ep=>ep.sources?.some(x=>x.url?.trim()));
        const acc=document.createElement("div");acc.className="season-accordion"+(isOpen?" open":"");
        acc.innerHTML=`<div class="season-header" data-toggle-s="${s.id}"><div class="season-header-left"><div class="season-toggle-icon">▶</div><div><div class="season-title">Temporada ${si+1}</div><div class="season-subtitle">${epN} ep(s) ${hasC?'· <span style="color:#4caf50">✓</span>':""}</div></div></div><div class="season-header-actions"><button type="button" class="season-mini-btn danger" data-rm-s="${s.id}" title="Remover">🗑️</button></div></div><div class="season-body"><div class="input-group"><label>🖼️ Capa da Temporada</label><label class="file-upload-box" id="scb_${s.id}"><span class="file-upload-label">📁 Escolher</span><input type="file" id="sci_${s.id}" accept="image/*" class="hidden"><img id="scp_${s.id}" src="${s.seasonCoverUrl||""}" class="file-preview-img ${s.seasonCoverUrl?"":"hidden"}"></label></div><label style="font-size:11px;font-weight:800;color:var(--primary);text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px;display:block">🎬 Episódios</label><div data-ep-list="${s.id}"></div><button type="button" class="add-btn-block" data-add-ep="${s.id}" style="margin-top:10px">+ Novo Episódio</button></div>`;
        c.appendChild(acc);
        renderEpList(s);
        q(`[data-toggle-s="${s.id}"]`,acc).onclick=e=>{if(e.target.closest("button"))return;syncInputs();uiSt.openS.has(s.id)?uiSt.openS.delete(s.id):uiSt.openS.add(s.id);renderSeasons();};
        q(`[data-rm-s="${s.id}"]`,acc).onclick=e=>{e.stopPropagation();if(!confirm("Remover temporada?"))return;syncInputs();const idx=creator.seasons.findIndex(x=>x.id===s.id);if(idx>=0){creator.seasons.splice(idx,1);uiSt.openS.delete(s.id);renderSeasons();}};
        q(`[data-add-ep="${s.id}"]`,acc).onclick=()=>{syncInputs();const ep={id:uid(),title:`Episódio ${(s.episodes?.length||0)+1}`,duration:"45m",thumbUrl:"",sources:[]};s.episodes=s.episodes||[];s.episodes.push(ep);uiSt.openE.add(ep.id);renderSeasons();};
        $(`scb_${s.id}`)?.addEventListener("click",ev=>{if(ev.target.closest("input"))return;ev.preventDefault();openCrop(`sci_${s.id}`,`scp_${s.id}`,16/9,u=>s.seasonCoverUrl=u);});
    });
}
...
// CROPPER
// ═══════════════════════════════
function openCrop(inputId,previewId,ar,onDone){
    const fi=$(inputId),pv=$(previewId);if(!fi)return;
    prevModal=!$("creatorModal")?.classList.contains("hidden")?"creatorModal":!$("profileModal")?.classList.contains("hidden")?"profileModal":null;
    cropTarget={pv,onDone};fi.value="";
...
function cleanCont(){let l=getContList();l=l.filter(i=>(Date.now()-(i.lw||0))<CONTINUE_EXPIRE);saveContList(l);return l;}
function saveContinue(item,extra="",epInfo=null){if(!curUid)return;let l=getContList().filter(i=>i.id!==item.id);l.unshift({id:item.id,title:item.title,type:item.type,cover:backdrop(item)||poster(item),dur:item.duration||extra,lw:Date.now(),epInfo});if(l.length>30)l.pop();saveContList(l);renderCont();}
...
function showSrcSel(){if(!pSel)return;ctrlLocked=true;setTouchZone(false);hideErr();pLoad?.classList.add("hidden");$("playerTitleDisplay").innerText=pMeta.main||"Escolha uma fonte";$("playerSubDisplay").innerText=pMeta.sub||"MasterFlix";$("playerProviderBadge")?.classList.add("hidden");$("playerSourceSelectorSub").innerText=`${sources.length} fonte(s)`;const list=$("playerSourceList");list.innerHTML="";sources.forEach((src,idx)=>{const btn=document.createElement("button");btn.type="button";btn.className="player-source-btn"+(idx===srcIdx&&vidUrl?" current":"");btn.tabIndex=0;const a=adapter(src.type),title=src.label||a.name,short=src.url?(src.url.length>55?src.url.slice(0,55)+"…":src.url):"",ct=idx===srcIdx&&vidUrl?`<span class="current-tag">Atual</span>`:"";btn.innerHTML=`<div class="source-btn-top"><span class="source-btn-title">${a.icon} ${title}${ct}</span><span class="source-btn-type" style="color:${a.color}">${a.name}</span></div><div class="source-btn-url">${short}</div>`;btn.onclick=()=>{hideSrcSel();srcIdx=idx;playSrc(idx);};list.appendChild(btn);});pSel.classList.remove("hidden");showCtrl(true);setTimeout(()=>list.querySelector(".player-source-btn")?.focus(),120);}
function hideSrcSel(){pSel?.classList.add("hidden");setTouchZone(true);unlockCtrl();}
function showErr(desc,name){$("playerErrorTitle").textContent=`Falha: ${name||"player"}`;$("playerErrorDesc").textContent=desc||"Tente outra fonte.";$("btnErrorTryAnother").style.display=sources.length>1?"":"none";pErr?.classList.remove("hidden");pLoad?.classList.add("hidden");setTouchZone(false);showCtrl(true);}
function hideErr(){pErr?.classList.add("hidden");setTouchZone(true);unlockCtrl();}
...
function initiatePlay(srcs,main,sub,itemId=null){if(!srcs?.length){showMsg("Nenhum link!");return;}sources=[...srcs];pMeta={main,sub};playingId=itemId;viewTracked=false;openModal("playerModal");$("btnChangeSource")?.classList.toggle("hidden",sources.length<=1);if(sources.length===1){srcIdx=0;playSrc(0);}else showSrcSel();}

function playSrc(idx){
    const src=sources[idx];if(!src?.url){showMsg("Link inválido!");return;}
    const a=adapter(src.type);const url=a.fmt(src.url);vidUrl=url;srcIdx=idx;
    ...
    pCont?.querySelectorAll("iframe,video").forEach(n=>n.remove());
    if(stretch){stretch=false;pCont?.classList.remove("stretch-mode");const b=$("btnToggleStretch");if(b){b.classList.remove("active");b.innerHTML="⇱";}}
    ...
    showCtrl();
    const tip=$("playerTip");if(tip){setTimeout(()=>{if(!pOpen())return;tip.textContent="Toque na tela para mostrar/ocultar controles";tip.classList.add("show");setTimeout(()=>tip.classList.remove("show"),4000);},2000);}
}

function closePlayer(){clearTimers();clearTimeout(errTimer);if(inFS()){try{document.exitFullscreen?.();}catch{}try{document.webkitExitFullscreen?.();}catch{}try{screen.orientation?.unlock?.();}catch{}}pCont?.querySelectorAll("iframe,video").forEach(n=>n.remove());pCont?.classList.remove("stretch-mode");pLoad?.classList.remove("hidden");hideErr();hideSrcSel();closeModal("playerModal");vidUrl="";sources=[];srcIdx=0;playingId=null;viewTracked=false;stretch=false;ctrlLocked=false;$("btnChangeSource")?.classList.add("hidden");$("playerProviderBadge")?.classList.add("hidden");const b=$("btnToggleStretch");if(b){b.classList.remove("active");b.innerHTML="⇱";}setCtrlVisible(true);setTouchZone(true);renderCont();}
$("btnClosePlayer")?.addEventListener("click",e=>{e.stopPropagation();closePlayer();});

// Player keyboard
document.addEventListener("keydown",e=>{if(!pOpen())return;if(e.key==="Escape"){closePlayer();return;}if(e.key==="f"||e.key==="F")$("btnToggleFullscreen")?.click();if(e.key==="s"||e.key==="S")$("btnToggleStretch")?.click();if(e.key==="r"||e.key==="R")$("btnReloadPlayer")?.click();if(e.key==="c"||e.key==="C")$("btnChangeSource")?.click();showCtrl();});

// ═══════════════════════════════
// FORM SAVE
// ═══════════════════════════════
$("mediaForm")?.addEventListener("submit",async e=>{
    e.preventDefault();syncInputs();
    if(!selGenres.length){showMsg("Selecione ao menos 1 gênero!");q('.creator-tab[data-tab="info"]')?.click();return;}
    const title=$("mediaTitle")?.value.trim()||"";if(!title){showMsg("Título!");q('.creator-tab[data-tab="info"]')?.click();return;}
    const editId=$("editMediaId")?.value;const ex=editId?catalog.find(m=>m.id===editId):null;
    let cv=$("mediaCoverPreview")?.src||"";let bd=$("mediaBackdropPreview")?.src||"";
    if(!validImg(cv))cv=ex?.coverUrl||"";if(!validImg(bd))bd=ex?.backdropUrl||"";
    const payload={type:creator.type,genres:[...selGenres],category:selGenres[0],title,year:$("mediaYear")?.value.trim()||"",duration:$("mediaDuration")?.value.trim()||"",description:$("mediaDesc")?.value.trim()||"",coverUrl:validImg(cv)?cv:"",backdropUrl:validImg(bd)?bd:""};
    if(creator.type==="movie"){const vs=creator.movieSrc.filter(s=>s.url?.trim());if(!vs.length){showMsg("Adicione ao menos 1 fonte!");q('.creator-tab[data-tab="content"]')?.click();return;}const used=vs.map(s=>s.type).filter(t=>t&&t!=="other");const dup=used.find((t,i)=>used.indexOf(t)!==i);if(dup){showMsg(`Duplicada: ${typeLabel(dup)}`);return;}payload.videoSources=vs.map(s=>({type:s.type||"other",label:s.label||typeLabel(s.type)||"Fonte",url:s.url.trim()}));payload.videoUrl=payload.videoSources[0].url;}else{
        const built=[];for(let si=0;si<creator.seasons.length;si++){const s=creator.seasons[si];const eps=[];for(let ei=0;ei<(s.episodes||[]).length;ei++){const ep=s.episodes[ei];const epS=(ep.sources||[]).filter(x=>x.url?.trim());const epU=epS.map(x=>x.type).filter(t=>t&&t!=="other");const epD=epU.find((t,i)=>epU.indexOf(t)!==i);if(epD){showMsg(`T${si+1}E${ei+1}: duplicada "${typeLabel(epD)}"`);return;}eps.push({title:ep.title?.trim()||`Episódio ${ei+1}`,duration:ep.duration?.trim()||"",thumbUrl:validImg(ep.thumbUrl)?ep.thumbUrl:"",videoSources:epS.map(x=>({type:x.type||"other",label:x.label||typeLabel(x.type)||"Fonte",url:x.url.trim()})),videoUrl:epS.length?epS[0].url.trim():""});}built.push({seasonNumber:si+1,seasonCoverUrl:validImg(s.seasonCoverUrl)?s.seasonCoverUrl:"",episodes:eps});}payload.seasons=built;}
    try{if(editId)await set(ref(db,"catalog/"+editId),payload);else await set(push(ref(db,"catalog")),payload);showMsg("Salvo! ✅","success");closeModal("creatorModal");resetCreator();await loadCatalog();}catch(err){showMsg("Erro: "+err.message);}
});
...
function renderAdmin(){const c=$("adminCatalogList");if(!c)return;const sv=norm($("adminSearchInput")?.value||"");c.innerHTML="";catalog.forEach(i=>{if(sv&&!norm(i.title).includes(sv))return;const d=document.createElement("div");d.className="admin-item";d.innerHTML=`<div><strong>${i.title}</strong><div style="font-size:10px;color:#888">${i.type==="movie"?"🎬 Filme":"📺 Série"}${i.year?" • "+i.year:""}</div></div><div style="display:flex;gap:6px"><button class="btn-secondary" onclick="editMedia('${i.id}')">✏️</button><button class="btn-danger" onclick="deleteMedia('${i.id}')">🗑️</button></div>`;c.appendChild(d);});}
...
function renderSugg(){const c=$("suggestionsAdminList");if(!c)return;c.innerHTML="";if(!allSugg.length){c.innerHTML=`<p style="text-align:center;color:#666;padding:30px">Vazio.</p>`;return;}allSugg.forEach(s=>{const d=document.createElement("div");d.className="suggestion-box";const date=s.timestamp?new Date(s.timestamp).toLocaleDateString("pt-BR"):"?";const cb=suggSelMode?`<input type="checkbox" class="suggestion-checkbox" data-id="${s.id}" ${suggSelIds.has(s.id)?"checked":""}>`:"";const del=!suggSelMode?`<button class="btn-danger" onclick="delSugg('${s.id}')">🗑️</button>`:"";const badge=s.type==="bug"?"🐛 BUG":"💡 SUG";d.innerHTML=`<div class="sg-header"><div style="display:flex;align-items:center;gap:8px">${cb}<div><div class="sg-user">${s.userName||"?"} ${badge}</div><div class="sg-email">${s.userEmail||""}</div></div></div><div style="display:flex;align-items:center;gap:6px"><span class="sg-date">${date}</span>${del}</div></div><div class="sg-text">${s.text}</div>`;if(suggSelMode){const chk=d.querySelector(".suggestion-checkbox");chk.onchange=()=>chk.checked?suggSelIds.add(s.id):suggSelIds.delete(s.id);}c.appendChild(d);});}
...
$("btnSaveProfile")?.addEventListener("click",async()=>{const u=auth.currentUser;if(!u)return;const tc=$("themeColorPicker")?.value||"#e50914";applyTheme(tc);const n=$("profileNameInput")?.value.trim()||"";const b=$("profileBioInput")?.value.trim()||"";const fg=$("profileFavGenreInput")?.value||"Ação";const ph=$("profilePhotoPreview")?.src||"";const bn=$("profileBannerPreview")?.src||"";const data={name:n,bio:b,favGenre:fg,photo:validImg(ph)?ph:"",banner:validImg(bn)?bn:"",themeColor:tc};try{await set(ref(db,"users/"+u.uid),data);if(n)cSet("name",n);if(b)cSet("bio",b);if(fg)cSet("fav_genre",fg);if(data.photo)cSet("avatar",data.photo);if(data.banner)cSet("banner",data.banner);cSet("theme",tc);updateAvatarUI(data);showMsg("Salvo! ✅","success");closeModal("profileModal");}catch(e){showMsg("Erro: "+e.message);}});
$("btnChangePasswordFromProfile")?.addEventListener("click",async()=>{const u=auth.currentUser;if(!u)return;if(!confirm(`Enviar link para ${u.email}?`))return;try{await sendPasswordResetEmail(auth,u.email);showMsg(`Link enviado!`,"success");}catch(e){showMsg("Erro: "+e.message);}});
async function doLogout(){try{const u=auth.currentUser;if(u){const did=getDevId();try{await remove(ref(db,`devices/${u.uid}/${did}`));}catch{}}stopHB();curUid=null;await signOut(auth);resetProfUI();document.documentElement.style.setProperty("--primary","#e50914");closeModal("profileModal");closeSB();catalog=[];isAdm=false;render();showMsg("Saiu! 👋","success");}catch{showMsg("Erro!");}}
...
function detectDev(){const ua=navigator.userAgent;let type="desktop",icon="💻",name="Desktop";if(/tablet|ipad/i.test(ua)&&!/mobile/i.test(ua)){type="tablet";icon="📱";name="Tablet";}else if(/mobile|android|iphone|ipod/i.test(ua)){type="mobile";icon="📱";name="Celular";}else if(/smarttv|googletv|appletv|hbbtv|tizen|webos|crkey|netcast|viera|roku|firetv|philipstv/i.test(ua)){type="tv";icon="📺";name="Smart TV";}let browser="Navegador";if(/chrome/i.test(ua)&&!/edg/i.test(ua))browser="Chrome";else if(/firefox/i.test(ua))browser="Firefox";else if(/safari/i.test(ua)&&!/chrome/i.test(ua))browser="Safari";else if(/edg/i.test(ua))browser="Edge";else if(/opera|opr/i.test(ua))browser="Opera";let os="?";if(/windows nt 10/i.test(ua))os="Windows 10/11";else if(/windows/i.test(ua))os="Windows";else if(/android/i.test(ua))os="Android";else if(/iphone|ipad|ipod/i.test(ua))os="iOS";else if(/mac/i.test(ua))os="macOS";else if(/linux/i.test(ua))os="Linux";return{type,icon,name,browser,os};}
...
async function openDevModal(){openModal("devicesModal");const list=$("devicesList"),info=$("devicesInfoText");if(list)list.innerHTML=`<div style="text-align:center;padding:30px"><div class="player-spinner" style="margin:0 auto;width:32px;height:32px"></div></div>`;if(info)info.textContent="Carregando...";const u=auth.currentUser;if(!u)return;try{const snap=await get(ref(db,`devices/${u.uid}`));const curId=getDevId();if(!snap.exists()){if(list)list.innerHTML=`<p style="text-align:center;color:#666;padding:30px">Nenhum dispositivo</p>`;return;}const devs=Object.values(snap.val()).sort((a,b)=>(b.lastActive||0)-(a.lastActive||0));if(info)info.textContent=`${devs.length} dispositivo(s)`;list.innerHTML="";devs.forEach(d=>{const isCur=d.deviceId===curId;const it=document.createElement("div");it.className="device-item"+(isCur?" current-device":"");it.innerHTML=`<div class="device-icon-big">${d.icon||"📱"}</div><div class="device-info"><div class="device-name">${d.name||"Dispositivo"} ${isCur?`<span class="device-current-tag">ESTE</span>`:""}</div><div class="device-meta"><span>🌐 ${d.browser||"?"}</span><span>💻 ${d.os||"?"}</span></div><div class="device-meta" style="margin-top:1px"><span>🕐 ${d.lastActive?timeAgo(d.lastActive):"?"}</span><span>📅 ${d.firstLogin?new Date(d.firstLogin).toLocaleDateString("pt-BR"):"?"}</span></div></div>${!isCur?`<button type="button" class="device-remove-btn" data-rm-dev="${d.deviceId}">✕</button>`:""}`;list.appendChild(it);});list.querySelectorAll("[data-rm-dev]").forEach(btn=>{btn.onclick=async()=>{if(!confirm("Encerrar sessão?"))return;await remove(ref(db,`devices/${u.uid}/${btn.dataset.rmDev}`));showMsg("Encerrada!","success");openDevModal();updateDevBadge();};});}catch(e){if(list)list.innerHTML=`<p style="text-align:center;color:#ff5252">${e.message}</p>`;}}
$("btnLogoutAllDevices")?.addEventListener("click",async()=>{const u=auth.currentUser;if(!u||!confirm("Encerrar todas as outras?"))return;const curId=getDevId();try{const snap=await get(ref(db,`devices/${u.uid}`));if(snap.exists())for(const did of Object.keys(snap.val()))if(did!==curId)await remove(ref(db,`devices/${u.uid}/${did}`));}catch{}showMsg("Encerradas!","success");openDevModal();updateDevBadge();});
...
function updateAuthMode(){const m=$("authModalBody");m?.classList.toggle("signup-mode",signUpMode);$("authSubtitle").innerText=signUpMode?"Crie sua conta":"Entre para continuar";$("btnAuthSubmit").innerText=signUpMode?"Criar Conta":"Entrar";const t=$("toggleAuthMode");if(t)t.innerHTML=signUpMode?'Já tem conta? <span style="color:var(--primary)">Entrar</span>':'Não tem conta? <span style="color:var(--primary)">Criar</span>';clearAuth();}
...
$("forgotPasswordForm")?.addEventListener("submit",async e=>{e.preventDefault();const email=$("forgotEmail")?.value.trim().toLowerCase();if(!email)return showMsg("Digite o e-mail!");const btn=$("btnSendResetEmail"),old=btn?.innerText;if(btn){btn.innerText="⏳...";btn.disabled=true;}try{await sendPasswordResetEmail(auth,email);closeModal("forgotPasswordModal");$("forgotSuccessEmail").textContent=email;openModal("forgotSuccessModal");}catch(err){showMsg(authErr(err.code));}if(btn){btn.innerText=old;btn.disabled=false;}});
$("authForm")?.addEventListener("submit",async e=>{e.preventDefault();if(authBusy)return;authBusy=true;const btn=$("btnAuthSubmit"),old=btn?.innerText;if(btn){btn.innerText="⏳...";btn.disabled=true;}const email=$("authEmail")?.value.trim().toLowerCase(),pass=$("authPassword")?.value,name=$("authDisplayName")?.value.trim()||"";if(!email||!pass||pass.length<6){showMsg("Preencha tudo! Senha mín. 6");authBusy=false;if(btn){btn.innerText=old;btn.disabled=false;}return;}if(signUpMode&&!name){showMsg("Nome de exibição!");authBusy=false;if(btn){btn.innerText=old;btn.disabled=false;}return;}try{if(signUpMode){if(auth.currentUser)await signOut(auth);const cred=await createUserWithEmailAndPassword(auth,email,pass);await set(ref(db,"users/"+cred.user.uid),{name,bio:"",favGenre:"Ação",photo:"",banner:"",themeColor:"#e50914",createdAt:Date.now()});showMsg("Conta criada! ✅","success");}else{if(auth.currentUser)await signOut(auth);await signInWithEmailAndPassword(auth,email,pass);showMsg("Bem-vindo! 🎬","success");}closeModal("authOverlay");clearAuth();}catch(err){showMsg(authErr(err.code));}authBusy=false;if(btn){btn.innerText=old;btn.disabled=false;}});
...
onAuthStateChanged(auth, async user=>{
    if(user){
        curUid=user.uid;resetProfUI();closeModal("authOverlay");clearAuth();
        isAdm=user.email.toLowerCase()===ADMIN_EMAIL.toLowerCase();
        ["profileAdminBadge","sidebarAdminItem","sidebarCreatorItem","sidebarSuggestionsAdminItem","sidebarStorageItem"].forEach(id=>$(id)?.classList.toggle("hidden",!isAdm));
        await loadProfile(user);
        await regDev(user);
        await Promise.all([loadCatalog(),loadViews()]);
    }else{
        stopHB();curUid=null;isAdm=false;resetProfUI();
        document.documentElement.style.setProperty("--primary","#e50914");
        openModal("authOverlay");clearAuth();
        ["sidebarAdminItem","sidebarCreatorItem","sidebarSuggestionsAdminItem","sidebarStorageItem","profileAdminBadge"].forEach(id=>$(id)?.classList.add("hidden"));
        signUpMode=false;updateAuthMode();
    }
});
